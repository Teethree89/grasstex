...
try {
  ...
  const initialStatus=await page.evaluate(()=>{
    ...
    const summarize=s=>({faction:s.faction,backend:s.animationBinding?.backend||null,mapped:s._skeletal?.mapped||0,asset:s._skeletal?.faction||null});
  ...
  if(initialStatus.us.backend!=='runtime-mixamo-driver'||initialStatus.ge.backend!=='runtime-mixamo-driver') throw new Error('Both factions did not bind runtime Mixamo backend: '+JSON.stringify(initialStatus));
  if(initialStatus.us.mapped<12||initialStatus.ge.mapped<12) throw new Error('Incomplete skeleton map: '+JSON.stringify(initialStatus));
  ...
  async function capturePose(faction,pose){
    ...
    if(info.backend!=='runtime-mixamo-driver'||info.mapped<12||info.meshCount<1||info.vertices<1)throw new Error('Skeletal subject not visibly instantiated: '+JSON.stringify(info));
