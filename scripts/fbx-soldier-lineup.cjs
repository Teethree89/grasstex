 ...
 *  3. Starts the live battle and captures each faction's rifleman, sergeant, engineer,
...
const MAC_CHROME = '/Volumes/Expanse/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
// Undefined lets Playwright use its own Chromium (cloud/CI) when the Mac binary is absent.
const CHROME = process.env.FBX_CHROME || (fs.existsSync(MAC_CHROME) ? MAC_CHROME : undefined);
...
(async () => {
  ...
  try {
    ...
    const shots = [];
    const expected = {
      us: { rifleman: ['us-paratrooper.fbx', 'm1-garand.fbx'], sergeant: ['us-captain.fbx', 'm1911a1.fbx'],
      ...
      ge: { rifleman: ['ge-paratrooper.fbx', 'kar98k.fbx'], sergeant: ['ge-captain.fbx', 'p38.fbx'],
