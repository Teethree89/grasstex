...
try{
  ...
  const result=await page.evaluate(async({fixedDt,simSeconds,suppliedPolicy})=>{
    ...
    const hot=root.BattleHotpathProfiler,meso=root.BattleMesoChurnProfiler,physical=root.BattlePhysicalPointProfiler;
    if(!hot||!meso||!physical)throw new Error('profilers failed to load');hot.reset();meso.reset();physical.reset();
  ...
  const c=result.meso?.counts||{},pc=result.physical?.counts||{},hotRows=result.hot?.rows||[],pick=label=>hotRows.find(r=>r.label===label)?.calls||0;
  const summary={
