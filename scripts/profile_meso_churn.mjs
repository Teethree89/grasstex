...
try{
  ...
  await page.addScriptTag({path:path.resolve('scripts/order-ingress-profiler.cjs')});
  await page.addScriptTag({path:path.resolve('scripts/resolver-order-mutation-profiler.cjs')});
  ...
  const result=await page.evaluate(async({fixedDt,simSeconds,suppliedPolicy})=>{
    ...
    const hot=root.BattleHotpathProfiler,meso=root.BattleMesoChurnProfiler,physical=root.BattlePhysicalPointProfiler,goalTransitions=root.BattleMovementGoalTransitionProfiler,ingress=root.BattleOrderIngressProfiler,mutation=root.BattleResolverOrderMutationProfiler;
    if(!hot||!meso||!physical||!goalTransitions||!ingress||!mutation)throw new Error('profilers failed to load');hot.reset();meso.reset();physical.reset();goalTransitions.reset();ingress.reset();mutation.reset();
    ...
    return{build:root.BATTLE_BUILD||null,policyRevision:suppliedPolicy?.revision||root.BattleAIPolicy.revision||0,simulatedSeconds:+(+sim.time||0).toFixed(2),wallSeconds:+wallSeconds.toFixed(3),realtimeMultiplier:wallSeconds>0?+((+sim.time||0)/wallSeconds).toFixed(2):0,steps,hot:hot.snapshot(120),meso:meso.snapshot(),physical:physical.snapshot(),goalTransitions:goalTransitions.snapshot(),orderIngress:ingress.snapshot(),resolverMutation:mutation.snapshot(),squadCommand:sim._squadCommandSummary||null,movementGoalStats:sim._movementGoalStats||null,tacticalRouteStats:sim._tacticalRouteStats||null};
  ...
  const c=result.meso?.counts||{},pc=result.physical?.counts||{},gc=result.goalTransitions?.counts||{},ic=result.orderIngress?.counts||{},mc=result.resolverMutation?.counts||{},hotRows=result.hot?.rows||[],pick=label=>hotRows.find(r=>r.label===label)?.calls||0;
  const summary={
    seed,battleType,wallSeconds:result.wallSeconds,
    goalShift:pick('navigation.physicalReplanReason.goal-shift'),squadStabilityGoalShift:pick('navigation.physicalReplan.goal-shift.owner.squad-stability'),
    fireteamProposals:c['proposeOrder.fireteam']||0,intentPublishes:c.intentPublishes||0,destinationChanges:c['fireteamDestination.changed']||0,signatureDestinationChanges:c['destinationChange.signatureChanged']||0,sameSignatureDestinationChanges:c['destinationChange.sameSignature']||0,slotDrift:c['destinationChange.slotDrift']||0,
    ...
    physicalDestinationChanges:pc['physicalDestination.changed']||0,physicalDirect:pc['producer.direct-pick']||0,physicalTacticalRoute:pc['producer.tactical-route']||0,physicalFiringStation:pc['producer.firing-station']||0,physicalSameHighLevelPick:pc['physicalChange.sameHighLevelPick']||0,physicalHighLevelPickChanged:pc['physicalChange.highLevelPickChanged']||0,
    squadStabilityPhysical:pc['squadStabilityPhysical.changed']||0,squadStabilityDirect:pc['squadStabilityProducer.direct-pick']||0,squadStabilityTacticalRoute:pc['squadStabilityProducer.tactical-route']||0,squadStabilitySameHighLevelPick:pc['squadStabilityPhysical.sameHighLevelPick']||0,squadStabilityHighLevelPickChanged:pc['squadStabilityPhysical.highLevelPickChanged']||0,
    movementGoalPhysicalChangesAfterWarmup:gc['physicalChanged.afterWarmup']||0,squadStabilityPhysicalAfterWarmup:gc['squadStabilityPhysical.afterWarmup']||0,squadStabilityIntraFormation:gc['squadStabilityPhysical.intraFormation']||0,squadStabilityIntraFormationPointChanged:gc['squadStabilityPhysical.intraFormation.pointChanged']||0,squadStabilityOwnershipReturns:gc['squadStabilityPhysical.ownershipReturn']||0,squadStabilityGoalMatchesFireteam:gc['squadStabilityPhysical.goalMatchesFireteam']||0,squadStabilityGoalDiffersFireteam:gc['squadStabilityPhysical.goalDiffersFireteam']||0,
    orderIngressCalls:ic.calls||0,orderIngressChanged:ic.orderChanged||0,orderIngressChangedMatchesFireteam:ic['orderChanged.matchesFireteam']||0,orderIngressChangedDiffersFireteam:ic['orderChanged.differsFireteam']||0,orderIngressRawDiffersFireteam:ic.rawDiffersFireteam||0,orderIngressProducers:result.orderIngress?.producers||[],
    resolverOrderChangedDuringResolve:mc.orderChangedDuringResolve||0,resolverOrderChangedPrivate:mc['orderChangedDuringResolve.privatePath']||0,resolverOrderChangedPublicStage:mc['orderChangedDuringResolve.publicStage']||0,resolverSameOrderPointMutation:mc['orderChangedDuringResolve.sameOrderObjectPointMutation']||0,resolverOrderReplacement:mc['orderChangedDuringResolve.orderReplacement']||0,
    resolverMutationCounts:mc,routePlanCreated:pc['tacticalRoute.planCreated']||0,routeStepAdvanced:pc['tacticalRoute.stepAdvanced']||0,movementGoalTransitions:(result.goalTransitions?.transitions||[]).slice(0,20)
  };
  ...
  console.log('RESOLVER_ORDER_MUTATION_COUNTS '+JSON.stringify(result.resolverMutation?.counts||{}));
  console.log('RESOLVER_ORDER_MUTATION_SAMPLES '+JSON.stringify((result.resolverMutation?.samples||[]).slice(0,30)));
  console.log('ORDER_INGRESS_COUNTS '+JSON.stringify(result.orderIngress?.counts||{}));
