<?php
/* 50webs production entrypoint for the Battle Sim / ww2fps AI laboratory v29.
   v29 gives individual combat a single owner: battle/engagement.js decides contact, cover, stance
...
/* Script URLs are busted by file mtime, which covers a normal deploy. CACHE_EPOCH is the manual
   override for the cases mtime cannot see: a rollback to older files, a mirror that rewrote
   timestamps, or a proxy holding a script under a query string it has already cached. Bump it to
   force every client to refetch every runtime regardless of mtime. */
$build='v29';$cacheEpoch='v29-1';
...
$coreScripts=array('soldier.js','weapons.js','obstacle-field.js','terrain-features.js','squad-ai.js','engagement.js','battle-sim.js');foreach($coreScripts as $file){$old='<script src="'.$file.'"></script>';$new='<script src="/grasstex/battle/'.$file.'?v='.$deployId.'&c='.rawurlencode($cacheEpoch).'"></script>';$body=str_replace($old,$new,$body);}
...
$preCommander=array('camera-controls.js','acoustics.js','scenario-generator.js','battle-navigation.js','town-objectives.js','module-registry.js','ai-policy.js','objective-system.js','battle-telemetry.js','commander-doctrine.js','commander-routes.js','commander-ai.js');$extras='';foreach($preCommander as $file)$extras.='<script src="/grasstex/battle/'.$file.'?v='.$deployId.'&c='.rawurlencode($cacheEpoch).'"></script>'."\n";
/* Modules load after commander: building hardpoints/armor/engineers may wrap generic behavior. */
foreach($moduleFiles as $file)$extras.='<script src="/grasstex/battle/modules/'.rawurlencode($file).'?v='.$deployId.'&c='.rawurlencode($cacheEpoch).'"></script>'."\n";
foreach(array('ai-trainer.js','battle-control.js') as $file)$extras.='<script src="/grasstex/battle/'.$file.'?v='.$deployId.'&c='.rawurlencode($cacheEpoch).'"></script>'."\n";
...
header('X-Grasstex-Deploy-Id: '.$deployId);header('X-Grasstex-Build: '.$build);header('X-Grasstex-Cache-Epoch: '.$cacheEpoch);header('X-Grasstex-Modules: '.count($moduleFiles));echo $body;
?>
