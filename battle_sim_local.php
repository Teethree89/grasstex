<?php
/* 50webs production entrypoint for the Battle Sim / ww2fps AI laboratory v23.
   v23 cache-busts the packaged soldier asset and loads it from the GitHub runtime source.
...
header('Pragma: no-cache');header('Expires: 0');header('Surrogate-Control: no-store');header('X-Grasstex-Source: modular-ai-lab-v23');
...
$assetBase='https://test.ivandpopov.com/grasstex/Assets/';$soldierAssetBase='https://raw.githubusercontent.com/Teethree89/grasstex/main/Assets/';$audioBase=$assetBase.'audio/';$apiBase='/grasstex/';
...
$bootstrap='<script>'.'window.BATTLE_BUILD="v23";'.'window.BATTLE_REF="local-'.$deployId.'";'.'window.BATTLE_ASSET_BASE='.json_encode($assetBase).';'.'window.BATTLE_SOLDIER_ASSET_BASE='.json_encode($soldierAssetBase).';'.'window.BATTLE_SOLDIER_ASSET_VERSION='.json_encode('v23-'.$deployId).';'.'window.BATTLE_AUDIO_BASE='.json_encode($audioBase).';'.'window.BATTLE_API_BASE='.json_encode($apiBase).';'.'window.BATTLE_AUDIO_MANIFEST='.json_encode($manifest).';'.'window.BATTLE_AI_POLICY='.json_encode($policyState).';'.'window.BATTLE_AI_MEMORY='.json_encode($memoryState).';'.'window.BATTLE_SCENARIO_SEED='.json_encode($requestedSeed).';'.'</script>';
...
header('X-Grasstex-Deploy-Id: '.$deployId);header('X-Grasstex-Build: v23');header('X-Grasstex-Modules: '.count($moduleFiles));echo $body;
?>
