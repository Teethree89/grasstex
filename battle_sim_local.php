<?php
/* 50webs production entrypoint for the Battle Sim / ww2fps AI laboratory v22.
   v22 adds WASD camera controls and keeps the per-match checkpoint/resource-reset trainer.
...
header('Pragma: no-cache');header('Expires: 0');header('Surrogate-Control: no-store');header('X-Grasstex-Source: modular-ai-lab-v22');
...
$bootstrap='<script>'.'window.BATTLE_BUILD="v22";'.'window.BATTLE_REF="local-'.$deployId.'";'.'window.BATTLE_ASSET_BASE='.json_encode($assetBase).';'.'window.BATTLE_SOLDIER_ASSET_BASE='.json_encode($assetBase).';'.'window.BATTLE_AUDIO_BASE='.json_encode($audioBase).';'.'window.BATTLE_API_BASE='.json_encode($apiBase).';'.'window.BATTLE_AUDIO_MANIFEST='.json_encode($manifest).';'.'window.BATTLE_AI_POLICY='.json_encode($policyState).';'.'window.BATTLE_AI_MEMORY='.json_encode($memoryState).';'.'window.BATTLE_SCENARIO_SEED='.json_encode($requestedSeed).';'.'</script>';
...
header('X-Grasstex-Deploy-Id: '.$deployId);header('X-Grasstex-Build: v22');header('X-Grasstex-Modules: '.count($moduleFiles));echo $body;
?>
