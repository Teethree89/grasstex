...
if ($body !== false && strlen($body) > 100 && stripos($body, '<html') !== false) {
    ...
    $bootstrap = '<script>window.BATTLE_BUILD="v23";window.BATTLE_REF='.json_encode($resolvedRef).';window.BATTLE_ASSET_BASE="https://test.ivandpopov.com/grasstex/Assets/";window.BATTLE_SOLDIER_ASSET_BASE='.json_encode($base . 'Assets/').';window.BATTLE_SOLDIER_ASSET_VERSION='.json_encode('v23-'.$resolvedRef).';window.BATTLE_AUDIO_BASE="https://test.ivandpopov.com/grasstex/Assets/audio/";window.BATTLE_API_BASE="/grasstex/";window.BATTLE_AUDIO_MANIFEST='.json_encode($manifest).';window.BATTLE_AI_POLICY='.json_encode($policyState).';window.BATTLE_AI_MEMORY='.json_encode($memoryState).';window.BATTLE_SCENARIO_SEED='.json_encode($requestedSeed).';</script>';
