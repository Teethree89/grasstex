...
if ($body !== false && strlen($body) > 100 && stripos($body, '<html') !== false) {
    ...
    foreach (array('soldier-animations.js','soldier.js','weapons.js','terrain-features.js','squad-ai.js','battle-sim.js') as $file) $body = str_replace('<script src="'.$file.'"></script>', '<script src="'.$base.'battle/'.$file.'?v='.$version.'"></script>', $body);
    $extras = '';
