/* Battle Sim v19 evolutionary tactical-policy trainer.
   ...
   This learns tactical parameters only; objective/victory rules remain fixed scenario rules. */
(function(root){
  'use strict';
  ...
  function fixedStep(sim,dt){
    var engine=sim.scene.getEngine(),oldGet=engine.getDeltaTime;
    engine.getDeltaTime=function(){return dt*1000;};
    sim._trainerStepActive=true;
    try{sim._frame();}finally{sim._trainerStepActive=false;engine.getDeltaTime=oldGet;}
  }
  function objectiveCounts(sim){
    var c=sim.objectiveControl||{};
    if(c.counts)return c.counts;
    return {us:c.us||0,ge:c.ge||0};
  }
  function scoreMatch(sim,candidateFaction){
    var enemy=other(candidateFaction),counts=objectiveCounts(sim),stats=sim.objectiveStats||{},caps=stats.capturesByFaction||{},pressure=stats.pressureSecondsByFaction||{};
    var score=0;
    ...
    score+=(sim.factions[candidateFaction].alive-sim.factions[enemy].alive)*1.25;
  ...
  function resultRecord(sim,candidateId,candidateFaction,score){
    ...
    return {candidateId:candidateId,candidateFaction:candidateFaction,winner:sim.winner||'none',time:+sim.time.toFixed(2),score:score,
      usAlive:sim.factions.us.alive,geAlive:sim.factions.ge.alive,usObjectives:counts.us||0,geObjectives:counts.ge||0,
      capturesByFaction:stats.capturesByFaction||{},pressureSecondsByFaction:stats.pressureSecondsByFaction||{}};
  }
  ...
  async function runMatch(sim,rawRestart,town,baseline,candidate,candidateId,candidateFaction,status,matchNo,totalMatches){
    ...
    while(!sim.winner&&sim.time<sim.timeLimit+1&&steps<maxSteps){
      ...
      for(var i=0;i<batch&&!sim.winner;i++){
        ...
        while(commandAccum>=root.BattleCommanderAI.commandTick&&!sim.winner){
          commandAccum-=root.BattleCommanderAI.commandTick;
          root.BattleCommanderAI.update(sim,town,root.BattleCommanderAI.commandTick);
        }
      }
    ...
    telemetry(sim,'policy-match-result',record);
    if(root.BattleTelemetry)root.BattleTelemetry.end(sim,'training-match-complete',record);
    return record;
  }
  ...
  async function train(sim,opts){
    ...
    sim._trainingRunning=true;
    var button=opts.button||null,status=opts.status||document.getElementById('aiTestStatus'),town=sim.scene.metadata&&sim.scene.metadata.battleTown;
    ...
    var rawRestart=sim._controlRawRestart||sim.restart.bind(sim);
    var saved={onFire:sim.onFire,onShot:sim.onShot,onCallout:sim.onCallout,onUpdate:sim.onUpdate,onWinner:sim.onWinner,timeScale:sim.timeScale,paused:sim.paused};
    sim.onFire=function(){};sim.onShot=function(){};sim.onCallout=function(){};sim.onUpdate=function(){};sim.onWinner=function(){};
    var baseline=root.BattleAIPolicy.get(),candidateCount=opts.candidates||5,strength=opts.mutationStrength==null?.16:+opts.mutationStrength,totalMatches=candidateCount*2;
    var candidates=[];
    for(var c=0;c<candidateCount;c++)candidates.push({id:'g'+(root.BattleAIPolicy.revision+1)+'-c'+(c+1),policy:root.BattleAIPolicy.mutate(baseline,strength),results:[]});
    telemetry(sim,'policy-training-start',{revision:root.BattleAIPolicy.revision,candidates:candidateCount,totalMatches:totalMatches,baseline:baseline});

    var matchNo=0,promotion=null,summaries=[];
    try{
      for(var i=0;i<candidates.length;i++){
        var candidate=candidates[i];
        telemetry(sim,'policy-candidate',{candidateId:candidate.id,policy:candidate.policy,distanceFromBaseline:root.BattleAIPolicy.distance(candidate.policy,baseline)});
        for(var sideIndex=0;sideIndex<2;sideIndex++){
          var side=sideIndex===0?'us':'ge';matchNo++;
          candidate.results.push(await runMatch(sim,rawRestart,town,baseline,candidate.policy,candidate.id,side,status,matchNo,totalMatches));
        }
        summaries.push(candidateSummary(candidate));
      }
      summaries.sort(function(a,b){return b.avgScore-a.avgScore;});
      var best=summaries[0],threshold=opts.promotionThreshold==null?12:+opts.promotionThreshold;
      var shouldPromote=!!(best&&best.avgScore>=threshold&&best.wins>=1&&best.losses<2);
      var trainingSummary={baselineRevision:root.BattleAIPolicy.revision,totalMatches:totalMatches,promotionThreshold:threshold,best:best?{id:best.id,avgScore:best.avgScore,wins:best.wins,losses:best.losses}:null,candidates:summaries.map(function(s){return {id:s.id,avgScore:s.avgScore,wins:s.wins,losses:s.losses};}),promoted:shouldPromote};
      telemetry(sim,'policy-training-summary',trainingSummary);

      if(shouldPromote){
        promotion=await root.BattleAIPolicy.persist(best.policy,{score:best.avgScore,matches:best.results.length,baselineScore:0,candidateId:best.id,generation:root.BattleAIPolicy.revision+1,sourceBuild:root.BATTLE_BUILD||'dev'});
        telemetry(sim,'policy-promoted',{candidateId:best.id,revision:promotion.revision,score:best.avgScore,policy:best.policy});
      }
      if(status){
        status.textContent=shouldPromote?
          ('AI learned policy r'+promotion.revision+' · '+best.id+' score '+best.avgScore+' · '+best.wins+'W/'+best.losses+'L'):
          ('AI kept policy r'+root.BattleAIPolicy.revision+' · best candidate '+(best?best.avgScore:'n/a')+' below '+threshold);
      }
      console.log('[TRAINING] v19 policy summary',trainingSummary,promotion||'no promotion');
      return {summary:trainingSummary,promotion:promotion,candidates:summaries};
    } finally {
      ...
      if(root.BattleTelemetry)root.BattleTelemetry.start(sim,'live',{afterTraining:true,policyRevision:root.BattleAIPolicy.revision});
