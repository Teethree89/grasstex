...
(function(root){
  ...
  function buildContext(sim,sq,chosen,enemy,p){
    var doc=doctrine(sim,sq.faction),status=chosen&&chosen.status||{},owner=status.owner||'neutral';
    var friendly=nearbyStrength(sim,sq.faction,p,90),hostile=nearbyStrength(sim,enemyFaction(sq.faction),p,90),ratio=friendly/Math.max(1,hostile);
    return{
      ...
      insideObjective:!!(chosen&&dist(p.x,p.z,chosen.point.x,chosen.point.z)<(+chosen.instance.def.radius||30)),
      underPressure:owner===sq.faction&&ownerPressure(status,sq.faction)>0,
      localRatio:ratio,friendlyStrength:friendly,enemyStrength:hostile
    };
  }
  function flankPoint(sq,chosen,town){
    var p=chosen.point,c=town&&town.center||{x:0,z:0},vx=p.x-c.x,vz=p.z-c.z,len=Math.hypot(vx,vz)||1;
    var side=(sq.commandRole==='left'||String(sq.id).length%2===0)?-1:1,off=Math.min(80,Math.max(35,(+chosen.instance.def.radius||30)*1.7));
    return{x:p.x+(-vz/len)*off*side,z:p.z+(vx/len)*off*side};
  }

  root.BattleCommanderDoctrine={
    FALLBACK:FALLBACK,FALLBACK_DOCTRINE:FALLBACK_DOCTRINE,
    genomeFor:genome,policyFor:policy,doctrineFor:doctrine,
