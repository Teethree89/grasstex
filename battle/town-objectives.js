...
(function(root){
  ...
  function buildBuilding(scene,heightAt,b,index,meshes){var y=heightAt(b.x,b.z),node=new BABYLON.TransformNode('building-'+b.id,scene);node.position.set(b.x,y,b.z);node.rotation.y=b.rot||0;meshes.push(node);var wallMat=mat(scene,'wallmat-'+b.id,index%3===0?WALL2:WALL),floorMat=mat(scene,'floormat-'+b.id,FLOOR);var slab=addBox(scene,node,'floor-'+b.id,b.w,.08,b.d,0,.04,0,floorMat);meshes.push(slab);buildWall(scene,node,b,'north',wallMat,meshes);buildWall(scene,node,b,'south',wallMat,meshes);buildWall(scene,node,b,'east',wallMat,meshes);buildWall(scene,node,b,'west',wallMat,meshes);return node;}
  function buildRoad(scene,heightAt,r,index,meshes){var dx=r.bx-r.ax,dz=r.bz-r.az,len=Math.hypot(dx,dz),cx=(r.ax+r.bx)/2,cz=(r.az+r.bz)/2,y=heightAt(cx,cz)+.025;var mesh=BABYLON.MeshBuilder.CreateBox('scenario-road-'+index,{width:r.width||8,height:.05,depth:len},scene);mesh.position.set(cx,y,cz);mesh.rotation.y=Math.atan2(dx,dz);mesh.material=mat(scene,'scenario-road-mat-'+index,ROAD);mesh.isPickable=false;meshes.push(mesh);}
  ...
  function renderScenario(scene,heightAt,scenario){
    disposeRender();activateTerrain(scenario);var meshes=[];
    for(var r=0;r<(scenario.roads||[]).length;r++)buildRoad(scene,heightAt,scenario.roads[r],r,meshes);for(var i=0;i<(scenario.buildings||[]).length;i++)buildBuilding(scene,heightAt,scenario.buildings[i],i,meshes);
    ...
    scenario.dispose=function(){for(var j=0;j<meshes.length;j++)try{meshes[j].dispose();}catch(_){}};scene.metadata=scene.metadata||{};scene.metadata.battleScenario=scenario;scene.metadata.battleTown=scenario;if(root.BattleNavigation)root.BattleNavigation.installScenario(scenario);currentRender=scenario;
  ...
  function regenerate(scene,heightAt,seed,meta,sim){
    if(currentBase&&currentBase._naturalDispose){try{currentBase._naturalDispose();}catch(_){}currentBase=null;}disposeRender();var scenario=root.BattleScenarioGenerator.create(seed,meta||{});activateTerrain(scenario);var base=scatterNatural(scene,heightAt,scenario,{}),rendered=renderScenario(scene,heightAt,scenario);wireBase(base,rendered);if(sim)sim.obstacles=base.obstacles||[];return rendered;
  }
