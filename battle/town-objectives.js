/* Central village/objective layer for Battle Sim v15.
...
(function(root){
  ...
  function addBuilding(scene,heightAt,meshes,obstacles,corners,id,x,z,w,d,h){
    ...
    // Several circles approximate the rectangular footprint for LOS, cover and steering.
    var nx=Math.max(2,Math.ceil(w/4)),nz=Math.max(2,Math.ceil(d/4));
    for(var ix=0;ix<nx;ix++)for(var iz=0;iz<nz;iz++){
      obstacles.push({x:x-w/2+(ix+.5)*w/nx,z:z-d/2+(iz+.5)*d/nz,radius:Math.max(w/nx,d/nz)*.62,cover:.28,type:'building',building:id});
    }
    var pad=3.2;
    corners.push({x:x-w/2-pad,z:z-d/2-pad,building:id,kind:'corner'});
    corners.push({x:x+w/2+pad,z:z-d/2-pad,building:id,kind:'corner'});
    corners.push({x:x-w/2-pad,z:z+d/2+pad,building:id,kind:'corner'});
    corners.push({x:x+w/2+pad,z:z+d/2+pad,building:id,kind:'corner'});
  }
  ...
  function buildTown(scene,heightAt){
    ...
    addBuilding(scene,heightAt,meshes,obstacles,corners,'NW1',-28,-26,18,14,7.2);
    addBuilding(scene,heightAt,meshes,obstacles,corners,'NE1', 29,-27,16,15,6.8);
    addBuilding(scene,heightAt,meshes,obstacles,corners,'NW2',-31, 20,15,18,7.5);
    addBuilding(scene,heightAt,meshes,obstacles,corners,'NE2', 31, 22,18,15,7.0);
    addBuilding(scene,heightAt,meshes,obstacles,corners,'W', -53,  2,15,20,6.5);
    addBuilding(scene,heightAt,meshes,obstacles,corners,'E',  53, -2,15,20,6.5);

    var sectors=[
      {id:'south-edge',x:0,z:-34,radius:23,label:'South edge'},
      {id:'square',x:0,z:0,radius:20,label:'Village square'},
      {id:'north-edge',x:0,z:34,radius:23,label:'North edge'}
    ];
    var routes={
      us:{
        left:[{x:-58,z:-58},{x:-58,z:-22},{x:-43,z:-10},{x:-18,z:0},{x:0,z:0},{x:0,z:34}],
        center:[{x:0,z:-58},{x:0,z:-34},{x:-8,z:-15},{x:0,z:0},{x:0,z:34}],
        right:[{x:58,z:-58},{x:58,z:-22},{x:43,z:-10},{x:18,z:0},{x:0,z:0},{x:0,z:34}],
        support:[{x:0,z:-62},{x:0,z:-46},{x:0,z:-38}]
      },
      ge:{
        left:[{x:58,z:58},{x:58,z:22},{x:43,z:10},{x:18,z:0},{x:0,z:0},{x:0,z:-34}],
        center:[{x:0,z:58},{x:0,z:34},{x:8,z:15},{x:0,z:0},{x:0,z:-34}],
    ...
    var town={meshes:meshes,obstacles:obstacles,corners:corners,sectors:sectors,routes:routes,center:{x:0,z:0},radius:70,
      dispose:function(){for(var i=0;i<meshes.length;i++)meshes[i].dispose();}};
    scene.metadata=scene.metadata||{};scene.metadata.battleTown=town;
    return town;
  }

  root.BattleTerrainFeatures.scatter=function(scene,heightAt,opts){
    opts=opts||{};
    // Reduce countryside clutter slightly so the village reads clearly.
    var base=oldScatter(scene,heightAt,Object.assign({},opts,{clumpCount:opts.clumpCount||17,hedgeRows:opts.hedgeRows||3}));
    var town=buildTown(scene,heightAt),oldDispose=base.dispose;
    base.obstacles=base.obstacles.concat(town.obstacles);base.town=town;
    base.dispose=function(){oldDispose&&oldDispose();town.dispose();};
    console.log('[TOWN] village built; buildings=6 sectors=3 obstacles='+town.obstacles.length);
    return base;
  };
