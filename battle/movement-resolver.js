...
(function (root) {
  ...
  function now(battle) {
    return battle && isFinite(+battle.time) ? +battle.time : 0;
  }
  function state(s) {
    if (!s._movementResolver)
      s._movementResolver = {
        order: null,
        combat: null,
        last: null,
  ...
  function signature(s) {
    ...
    return [
      q.commandPhase || '',
      q.targetObjective || '',
      (q._engagementPlan && q._engagementPlan.serial) || 0,
  ...
  function metrics(b) {
    return (
      b._movementGoalStats ||
      (b._movementGoalStats = {
        combatIntentRequests: 0,
        combatIntentCoalesced: 0,
        requests: 0,
        actualChanges: 0,
        equivalentRequestsIgnored: 0,
        hysteresisRetains: 0,
        ...
        blockedGoalFallbacks: 0,
        illegalGoalsUnresolved: 0,
        overridesByPriority: {},
        bySource: {}
      })
    );
  }
  function count(b, key, source) {
    var m = metrics(b);
    m[key] = (m[key] || 0) + 1;
    if (source) {
      var row = m.bySource[source] || (m.bySource[source] = { requests: 0, changes: 0 });
      if (key === 'requests') row.requests++;
  ...
  function proposal(owner, pt, battle, kind, urgent, ttl) {
    pt = point(pt);
    if (!pt) return null;
    return {
      owner: owner,
      ...
      urgent: !!urgent,
      issuedAt: now(battle),
      until: now(battle) + (ttl == null ? Infinity : ttl)
    };
  }
  ...
  function projectBlockedPoint(soldier, battle, raw, metric, source) {
    ...
    if (!N || typeof N.movementClear !== 'function' || N.movementClear(p, p)) return p;
    var start = point(soldier && soldier.root && soldier.root.position);
    if (!start) return p;
    var total = distance(p, start);
    if (!isFinite(total) || total < 0.02) return p;

    var P = root.BattleNavigationPhysicality,
      overlaps = null,
      margin = P && isFinite(+P.navMargin) ? +P.navMargin : 0;
    if (P && typeof P.footprints === 'function' && typeof P.shapeContains === 'function') {
      try {
        var fps = P.footprints(battle) || [];
        overlaps = [];
    ...
    function at(d) {
      var u = Math.max(0, Math.min(1, d / total));
      return { x: p.x + (start.x - p.x) * u, z: p.z + (start.z - p.z) * u };
    }
    function blocked(q) {
    ...
    var lo = 0,
      hi = Math.min(0.25, total),
      q = at(hi);
    while (hi < total && blocked(q)) {
      lo = hi;
    ...
    if (blocked(q)) {
      if (N.movementClear(start, start)) {
        ...
        count(battle, 'blockedGoalFallbacks', source);
        return start;
      }
    ...
    for (var k = 0; k < 12; k++) {
      var mid = (lo + hi) * 0.5,
        mq = at(mid);
      if (blocked(mq)) lo = mid;
      else hi = mid;
    }
    var d = Math.min(total, hi + 0.12),
      out = at(d);
    ...
    while (d < total && !N.movementClear(out, out)) {
      d = Math.min(total, d + 0.12);
      out = at(d);
    }
  ...
  function legalizeGoal(soldier, battle, raw, kind, source) {
    var p = point(raw);
    if (!p || !battle || PRECISE_GOALS[kind]) return p;
    var P = root.BattleNavigationPhysicality,
      out;
    ...
    if (kind === 'formation' && P && typeof P.resolveStandGoal === 'function') {
      ...
      if (out && distance(out, p) > ORDER_WRITE_EPS) {
        ...
        count(battle, 'formationEndpointResolutions', source);
        if (soldier)
          soldier._movementEndpointResolution = {
            kind: 'formation',
            intent: { x: p.x, z: p.z },
            point: { x: out.x, z: out.z },
            at: now(battle)
          };
      } else if (soldier) delete soldier._movementEndpointResolution;
      var N = root.BattleNavigation;
      if (out && (!N || typeof N.movementClear !== 'function' || N.movementClear(out, out)))
    ...
    if (cache && cache.kind === kind && distance(cache.raw, p) <= ORDER_WRITE_EPS)
      return { x: cache.point.x, z: cache.point.z };
    out = projectBlockedPoint(soldier, battle, p, 'goalLegalizations', source);
    if (soldier)
      soldier._movementLegalGoalCache = {
        kind: kind,
        raw: { x: p.x, z: p.z },
        point: { x: out.x, z: out.z }
      };
  ...
  function proposeOrder(soldier, next, battle, urgent) {
    ...
    var st = state(soldier),
      team = point(soldier._fireteamDestination),
    ...
    var sig = signature(soldier),
      ...
      oldIntent = old && point(old.intentPoint || old.point);
    if (
      old &&
      old.owner === source &&
      old.signature === sig &&
      oldIntent &&
      distance(oldIntent, raw) <= ORDER_WRITE_EPS &&
      cur &&
      distance(cur, old.point) <= ORDER_WRITE_EPS
    ) {
      old.urgent = !!urgent;
      old.until = Infinity;
      return old;
    ...
    st.order = proposal(source, p, battle, 'formation', urgent, Infinity);
    st.order.signature = sig;
    st.order.reason = 'squad intent';
  ...
  function proposeCombat(soldier, next, battle, kind, ttl, meta) {
    ...
    if (old && valid(soldier, old, battle)) {
      if (p.priority < priority(old.kind, soldier)) {
        count(battle, 'lowerPriorityRejected');
        return old;
      }
      if (old.kind === p.kind && old.owner === p.owner && distance(old.point, p.point) <= tolerance(p.kind)) {
        old.until = p.until;
        old.intentPoint = { x: raw.x, z: raw.z };
        count(battle, 'equivalentRequestsIgnored');
        return old;
      }
      if (
        !meta.material &&
        ...
        count(battle, 'hysteresisRetains');
        return old;
      }
    }
    st.combat = p;
  ...
  function choose(soldier, battle) {
    ...
    if (sq.commandPhase === 'regroup') {
      ...
      return proposal(
        'squad-command',
        (st.order && st.order.point) || soldier.orderDestination || sq.rally,
        battle,
        'regroup',
        true,
        Infinity
    ...
    return (
      st.order || proposal('squad-orders', soldier.orderDestination, battle, 'formation', false, Infinity)
    );
  }
  function tacticalWaypoint(soldier, battle, pick) {
    ...
    try {
      return T.resolve(soldier, battle, pick) || null;
    } catch (err) {
      if (root.console && console.warn) console.warn('[MOVE] tactical route failed', err);
      return null;
    }
  }
  function resolve(soldier, battle) {
    ...
    var st = state(soldier),
      pick = choose(soldier, battle);
    if (!pick) return null;
    pick.signature = signature(soldier);
    var previous = st.goal,
      oldValid = previous && valid(soldier, previous, battle);
    if (
      previous &&
      previous.kind === pick.kind &&
      previous.signature === pick.signature &&
      distance(previous.point, pick.point) <= tolerance(pick.kind)
    ) {
    ...
    var routed = tacticalWaypoint(soldier, battle, pick),
      rawPhysical = point(routed && routed.point) || pick.point;
    ...
    var physical = projectBlockedPoint(
      soldier,
      battle,
      rawPhysical,
      routed ? 'tacticalWaypointBacktracks' : null,
      pick.owner
    );
    if (routed && distance(rawPhysical, physical) > ORDER_WRITE_EPS) {
      var plan = soldier._tacticalRoute,
        idx = isFinite(+routed.step) ? +routed.step : plan && isFinite(+plan.index) ? +plan.index : null;
      if (plan && plan.steps && idx != null && plan.steps[idx])
        plan.steps[idx] = { x: physical.x, z: physical.z };
      routed.point = { x: physical.x, z: physical.z };
    }

    var space = root.BattleSoldierPersonalSpace;
    if (space && space.resolveDestination)
      physical = space.resolveDestination(battle, soldier, physical, pick.kind, !!routed);

    var epsilon =
      routed || soldier._personalSpaceDestination || !previous || previous.kind !== pick.kind
        ? 0.1
        : tolerance(pick.kind);
    var current = point(soldier.destination),
      atCurrent =
        current && distance({ x: +soldier.root.position.x, z: +soldier.root.position.z }, current) < ARRIVAL,
      changed = !current || distance(current, physical) > epsilon,
      canChange =
        pick.urgent || !!routed || atCurrent || now(battle) >= (soldier._destinationCommitUntil || 0);
    if (changed && canChange) {
      count(battle, 'actualChanges', pick.owner);
      var pk = String(priority(pick.kind, soldier));
      metrics(battle).overridesByPriority[pk] = (metrics(battle).overridesByPriority[pk] || 0) + 1;
      var route = soldier._physicalPath,
        trace = {
          time: now(battle),
          soldier: soldier.id,
          oldDestination: current,
          newDestination: physical,
          source: pick.owner,
          reason: pick.reason || pick.kind,
          kind: pick.kind,
          priority: priority(pick.kind, soldier),
          commandPhase: soldier.squad && soldier.squad.commandPhase,
          engagementState: soldier.eng && soldier.eng.state,
          oldGoalValid: !!oldValid,
          previousRoute: route
            ? {
                createdAt: route.createdAt,
                blocked: route.blocked,
                remaining: route.points && route.points.length
              }
            : null,
          inContact: !!(soldier.squad && soldier.squad.inContact),
          localAvoidance: !!(
            soldier._movementYieldUntil > now(battle) || soldier._separatedAt > now(battle) - 1
          ),
          destinationDistanceDelta: current
            ? distance(soldier.root.position, physical) - distance(soldier.root.position, current)
            : null
        };
      (st.history || (st.history = [])).push(trace);
      if (st.history.length > 32) st.history.shift();
      soldier._movementResolvedOwner = 'movement-resolver';
    ...
    st.last = {
      ...
      until: pick.until,
      point: { x: actual.x, z: actual.z },
      intentPoint: { x: intent.x, z: intent.z },
      tacticalReason: (routed && routed.reason) || null,
      tacticalStep: routed ? { index: routed.step, total: routed.total } : null
    };
    if (pick.owner === 'engagement' || pick.owner === 'tactical-positions') st.combatWins++;
    else st.orderWins++;
    return st.last;
  }
  function resetSoldier(soldier) {
    if (soldier) {
      delete soldier._movementResolver;
      delete soldier._movementTacticalReason;
      delete soldier._tacticalRoute;
      delete soldier._movementLegalGoalCache;
      delete soldier._movementEndpointResolution;
      delete soldier._personalSpaceDestination;
    }
  }
    ...
    var out = Object.assign(
        {
          ...
          tacticalWins: 0,
          bySoldier: [],
          highestChurnSoldier: null
        },
        metrics(sim)
      ),
      roster = (sim && sim._roster) || {},
      active = 0;
    ['us', 'ge'].forEach(function (f) {
      (roster[f] || []).forEach(function (s) {
        var st = s._movementResolver;
        if (!st) return;
        out.changed += st.changes || 0;
        var row = { id: s.id, faction: f, changes: st.changes || 0, requests: st.requests || 0 };
        out.bySoldier.push(row);
        if (!out.highestChurnSoldier || row.changes > out.highestChurnSoldier.changes)
          out.highestChurnSoldier = row;
        if (!s.dead) {
          active++;
          var last = st.last;
          if (last) {
            if (last.kind === 'formation') out.orders++;
            else out.combat++;
            out.byKind[last.kind] = (out.byKind[last.kind] || 0) + 1;
          }
          out.stickyCombatWins += st.stickyCombatWins || 0;
          out.tacticalWins += st.tacticalWins || 0;
        }
      });
    });
    out.averageChangesPerActiveSoldier = active
      ? out.bySoldier
          .filter(function (r) {
            return (roster[r.faction] || []).some(function (s) {
              return s.id === r.id && !s.dead;
            });
          })
          .reduce(function (n, r) {
            return n + r.changes;
