...
(function(root){
  ...
  var queue=[],active=false,sessionId=null,mode='live',flushTimer=null,seq=0,consoleLogging=!!root.BATTLE_DEBUG_TELEMETRY;
  ...
  function record(type,data,sim,internal){
    ...
    queue.push(e);if(consoleLogging&&(type.indexOf('decision')===0||type.indexOf('objective')===0||type.indexOf('policy-')===0||type==='reinforcement'||type==='module-spawn'||type==='battle-end'||type==='training-result'))console.log('[AI]',type,JSON.stringify(e.data));if(queue.length>=20)flush(false);
  }
  ...
  function state(){return{active:active,sessionId:sessionId,mode:mode,queued:queue.length,seq:seq,endpoint:ENDPOINT,consoleLogging:consoleLogging};}
  ...
  root.BattleTelemetry={start:start,ensure:ensure,record:record,end:end,checkpoint:checkpoint,flush:flush,setConsoleLogging:setConsoleLogging,state:state};console.log('[TELEMETRY] runtime v20 loaded');
