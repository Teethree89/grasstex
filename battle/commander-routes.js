...
(function(root){
  'use strict';
  var D=function(){return root.BattleCommanderDoctrine;};

  function rolePlan(sim,faction,index,count){
    var doc=D().doctrineFor(sim,faction),reserveCount=Math.round(count*doc.reserveFraction);
    if(reserveCount>0&&index>=count-reserveCount)return'reserve';
    var cycle=doc.flankPreference>.66?['left','right','center','left','right']:
              doc.flankPreference<.25?['center','support','center','left','right']:
  ...
  root.BattleCommanderRoutes={rolePlan:rolePlan,routeFor:routeFor,assignSquad:assignSquad,ensureAssignments:ensureAssignments,initForce:initForce};
