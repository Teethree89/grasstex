...
function clearPlan(s,b,cancelled){if(!s||!s._tacticalRoute)return;if(cancelled){routeStats(b).cancelledPlans++;s._tacticalRouteCooldownUntil=Math.max(+s._tacticalRouteCooldownUntil||0,b.time+CANCEL_COOLDOWN);}if(root.BattleCoverPositions)root.BattleCoverPositions.release(s,b,'route');delete s._tacticalRoute;}
...
function protectivePoint(s,b,pick,threat){
  var C=root.BattleCoverPositions,N=root.BattleNavigation,here=pos(s);if(!C||!here||!threat)return null;
  var candidates=C.candidates(s,b,threat,COVER_SEARCH),direct=dist(here,pick.point),cheap=[];
  for(var i=0;i<candidates.length;i++){
    var pt=candidates[i],straight=dist(here,pt),remaining=dist(pt,pick.point);
    if(straight<2||dist(pt,threat)<10||straight+remaining>direct+MAX_DETOUR)continue;
    cheap.push({point:pt,q:pt.quality,remaining:remaining,score:straight+remaining*.12+pt.quality*24});
  }
  cheap.sort(function(a,c){return a.score-c.score;});var best=null,bestScore=Infinity,limit=Math.min(MAX_COVER_PATH_CANDIDATES,cheap.length);
  for(var j=0;j<limit;j++){
    var cc=cheap[j],toCover;
    if(N&&N.movementClear&&N.movementClear(here,cc.point))toCover=dist(here,cc.point);
    else{var path=navPath(here,cc.point,b);if(!path||!path.length||dist(path[path.length-1],cc.point)>.35)continue;toCover=pathLength(here,path);}
    if(toCover+cc.remaining>direct+MAX_DETOUR)continue;
    var actual=toCover+cc.remaining*.12+cc.q*24;if(actual<bestScore){bestScore=actual;best=cc.point;}
  }
