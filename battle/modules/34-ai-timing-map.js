...
function loopData(){var v=values(),capture=v.capture.map(fmt).join(' / ');return[
  ...
  {id:'replan-health',title:'Objective progress → replan health signal',kind:'watch',keys:['system:objective','system:commander','system:squad'],seq:['sample assignments','watch objective state','no progress for '+fmt(v.replanAfter),'flag replan due','export with diagnostics'],note:'This is an observability gate, not an automatic command override. It exposes stalled assault plans and missing role/target intent so a future commander replan can be evaluated safely.'},
