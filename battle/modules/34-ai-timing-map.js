...
function values(){
  ...
  return{
    commandTick:n(cmd.commandTick,.45),objectiveVictory:n(cmd.objectiveHoldWin,35),
    ...
    alertHold:n(eng.ALERT_HOLD,4.5),boundCycle:n(eng.BOUND_CYCLE,9),boundDuration:n(eng.BOUND_DURATION,3.6),suppressPause:n(eng.SUPPRESS_PAUSE,2.6),
...
function decorate(){
  ...
  ensureNodeTiming('system:commander','<div class="atm-label">CADENCE / HOLDS</div><div class="atm-chips">'+chip('tick',fmt(v.commandTick),'fixed','', 'Commander reevaluates on this cadence')+chip('support delay',fmt(v.supportDelay),'editable','param:0','Editable policy parameter')+chip('regroup',fmt(v.regroupHold),'editable','param:1','Editable policy parameter')+chip('corner',fmt(v.cornerHold),'editable','param:1','Editable policy parameter')+'</div>','c|'+[v.commandTick,v.supportDelay,v.regroupHold,v.cornerHold].join('|'));
...
function loopData(){var v=values(),capture=v.capture.map(fmt).join(' / ');return[
  ...
  {id:'objective',title:'Capture → secure → release',kind:'intended',keys:['system:objective','system:commander','system:squad','defense:defense'],seq:['capture '+capture,'enter defend','secure window '+fmt(v.postCaptureSecure),'if clear: release','commander resumes intent'],note:'The capture-zone module owns the short post-capture secure window; it is not a two-minute hold rule.'},
  ...
  {id:'engineer',title:'Secure ground → engineer → prepared defense',kind:'intended',keys:['defense:engineer','defense:defense','system:squad','system:engagement'],seq:['friendly owned objective','engineer safe / out of contact '+fmt(v.engineerBuild),'build work','claim prepared posts','up to '+v.engineerMax+' runtime works'],note:'After construction the squad is assigned prepared posts. Strategic garrison intent is persistent unless another system releases it.'},
