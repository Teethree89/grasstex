...
function sample(sim){
  ...
  ['us','ge'].forEach(function(f){var squads=sim.factions&&sim.factions[f]&&sim.factions[f].squads||[];for(var i=0;i<squads.length;i++){
    ...
    for(var j=0;j<members.length;j++){var s=members[j],dest=s.destination?{x:+s.destination.x||0,z:+s.destination.z||0}:null,sp={time:sim.time,pos:{x:+s.root.position.x||0,z:+s.root.position.z||0},dest:dest,destSig:pointSig(dest,1.8),eng:s.eng&&s.eng.state||s.state||'',phase:ss.phase,rule:ss.rule,inContact:ss.inContact};var hh=pushHistory(st.soldiers,f+':'+sq.id+':'+s.id,sp);detectSoldier(sim,s,sq,hh);}
  }});renderLoopPanel();
}
