...
var ARRIVE=.95,THREAT_AGE=10,COVER_SEARCH=30,GOOD_COVER=.88,MAX_DETOUR=42,
    ...
    COVER_REPLAN_COOLDOWN=3,CANCEL_COOLDOWN=.75,MAX_COVER_PATH_CANDIDATES=3;
...
/* ---- committed tactical micro-route ------------------------------------------------------- */
...
/* ---- conservative movement progress ------------------------------------------------------- */
...
function observe(s,b,p,pick){
  p=point(p);if(!s||!b||!p||!expected(s,pick,b)){if(s&&s._movementProgress){s._movementProgress.samples=[];s._movementProgress.stuck=false;}return null;}var st=prog(s),owner=String(pick&&pick.owner||''),kind=String(pick&&pick.kind||'');expire(st,b.time);
  if(!st.goal||dist(st.goal,p)>2.4||st.owner!==owner||st.kind!==kind)resetEpisode(s,st,p,pick,b);var h=pos(s);if(!h)return null;var last=st.samples[st.samples.length-1];if(!last||b.time-last.t>=SAMPLE_DT)st.samples.push({t:b.time,x:h.x,z:h.z});while(st.samples.length&&b.time-st.samples[0].t>WINDOW*1.6)st.samples.shift();if(dist(h,st.goal)<=FAR){clearFailuresNear(s,b,st.goal);st.samples=[];st.stuck=false;return null;}var first=st.samples[0];if(!first||b.time-first.t<WINDOW||b.time-st.since<WINDOW)return null;
  var net=Math.hypot(h.x-first.x,h.z-first.z),idx=routeIndex(s),rem=routeRemaining(s),odo=0;for(var i=1;i<st.samples.length;i++)odo+=Math.hypot(st.samples[i].x-st.samples[i-1].x,st.samples[i].z-st.samples[i-1].z);if(net>=NET||idx>(st.routeIndex||0)||(rem!=null&&st.remaining!=null&&rem<st.remaining)||odo>=NET){st.confirm=0;st.stuck=false;st.routeIndex=idx;st.remaining=rem;return null;}
  st.confirm++;if(st.confirm<CONFIRMS)return null;st.confirm=0;if(!st.stuck){st.stuck=true;progressStats(b).stuckDetections++;}
  if(!st.recovered){st.recovered=true;st.rebuildAt=b.time;st.samples=[{t:b.time,x:h.x,z:h.z}];st.since=b.time;progressStats(b).recoveryAttempts++;progressStats(b).routeRebuilds++;return{rebuild:true,round:1};}
  if(b.time-st.rebuildAt<OBSERVE)return null;
  /* Only a combat candidate with a real alternative is allowed to become unreachable. Formation,
     retreat and regroup remain valid command intent and are never abandoned by this observer. */
  if(GATED[kind]){noteFailure(s,b,st.goal,'no-progress');s._movementGoalUnreachable=true;progressStats(b).unreachableFlags++;progressStats(b).alternateApproaches++;st.samples=[];st.stuck=false;return{rebuild:true,alternate:true,unreachable:true,round:2};}
  st.rebuildAt=b.time;st.samples=[{t:b.time,x:h.x,z:h.z}];st.since=b.time;st.stuck=false;progressStats(b).recoveryAttempts++;progressStats(b).routeRebuilds++;return{rebuild:true,round:2};
}
