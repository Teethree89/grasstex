...
/* Formation advance may be wide, but an assault order should not make a man recover a huge lateral
   slot or walk backwards. This changes the command proposal, never soldier.destination. */
function compactOrder(sim,s,q,ax){
  var mr=s&&s._movementResolver,p=mr&&mr.order,pt=p&&point(p.point);if(!pt)return;
  var dx=pt.x-ax.anchor.x,dz=pt.z-ax.anchor.z,fw=dx*ax.fx+dz*ax.fz,lat=dx*ax.rx+dz*ax.rz,max=q.inContact?MAX_LATERAL_CONTACT:MAX_LATERAL_ADVANCE;
  var nfw=Math.max(-MAX_FORMATION_BACK,fw),nlat=Math.max(-max,Math.min(max,lat)),here=pos(s);
  if(here&&!q.inContact){var hx=here.x-ax.anchor.x,hz=here.z-ax.anchor.z,hf=hx*ax.fx+hz*ax.fz;nfw=Math.max(nfw,hf-MAX_ORDER_BACK);}
  if(Math.abs(nfw-fw)<.01&&Math.abs(nlat-lat)<.01)return;
  p.point={x:ax.anchor.x+ax.fx*nfw+ax.rx*nlat,z:ax.anchor.z+ax.fz*nfw+ax.rz*nlat};if(s.orderDestination){s.orderDestination.x=p.point.x;s.orderDestination.z=p.point.z;}bump(sim,s,'orderClamps');
}
function allowCover(s,sim,pt){
  var q=s&&s.squad;if(!q||q.commandPhase!=='assault'||q.state==='retreat'||(+s.suppressedUntil||0)>sim.time)return true;var ax=axis(sim,q),here=pos(s);if(!ax||!here)return true;
  var fw=(pt.x-here.x)*ax.fx+(pt.z-here.z)*ax.fz;if(fw>=MIN_COVER_FORWARD)return true;bump(sim,s,'coverRejects');if(fw>=-.25)bump(sim,s,'lateralCoverRejects');return false;
}
...
/* Fixed no-cover fireteam bound. The goal is chosen once per bound token; target flicker cannot
   move it. */
...
function maintainPush(s,b){
  var r=s._assaultBoundPush;if(r&&r.token!==boundToken(s)){clearPush(s,b,true);r=null;}if(boundUnsafe(s,b)){clearPush(s,b,!!r);return;}
  var combat=s._movementResolver&&s._movementResolver.combat;if(!r){if(!(combat&&combat.kind==='hold'))return;r=startPush(s,b);if(!r)return;}
  var h=pos(s);if(dist(h,r.goal)<=PUSH_ARRIVAL){r.completed=true;bump(b,s,'pushCompletions');delete s._assaultBoundPush;if(root.BattleMovementProgress)root.BattleMovementProgress.clearFailuresNear(s,b,r.goal);return;}
  if(s._movementGoalUnreachable){s._movementGoalUnreachable=false;if(root.BattleMovementProgress)root.BattleMovementProgress.noteFailure(s,b,r.goal,'bound-unreachable');clearPush(s,b,true);return;}
  if(root.BattleMovementProgress&&!root.BattleMovementProgress.candidateAllowed(s,b,r.goal)){clearPush(s,b,true);return;}
  s._combatUrgentUntil=Math.max(+s._combatUrgentUntil||0,b.time+.5);s.prone=false;s.crawling=false;s.tacticalCrouch=true;
  root.BattleMovementResolver.proposeCombat(s,r.goal,b,'assault-bound-push',.8,{source:'combat-mobility',reason:'authorized fireteam bound'});
}

/* Suppressed men get one committed urgent cover move; squadmates react once to a fresh shared
   threat sector. */
...
function reactShared(s,b,e){
  var q=s&&s.squad;if(!q||!q.inContact){if(e._sharedContactQuietAt==null)e._sharedContactQuietAt=b.time;if(e._sharedContactAware&&b.time-e._sharedContactQuietAt>=SHARED_RESET_QUIET){e._sharedContactAware=false;e._sharedContactSector=null;e._sharedContactReactedAt=-999;}return;}
  e._sharedContactQuietAt=null;if(s.target||e.state!=='advance'||s.reloading||s.clearingStoppage)return;var c=contact(s,b);if(!c)return;var h=pos(s),aim={x:+c.x||0,z:+c.z||0};if(!h)return;var sec=sectorFor(s,aim);if(e._sharedContactAware&&sectorDistance(e._sharedContactSector,sec)<=1){bump(b,s,'sharedContactRepeatBlocks');return;}
