...
function decorateCore(){
  var command=graphNode('system:commander'),squad=graphNode('system:squad'),eng=graphNode('system:engagement'),soldier=graphNode('system:soldier');
  ...
  if(eng){var he=eng.querySelector('.ag-head>span'),be=eng.querySelector('.ag-head small');if(he)he.textContent='Engagement';if(be)be.textContent='individual combat owner';ownerBox(eng,'<span class="how">OWNS HOW EACH MAN FIGHTS</span><br><b>cover · stance · bound · fire · personal combat movement</b><br>This remains the single authority for individual combat decisions.','eng-v1');}
  if(soldier)ownerBox(soldier,'<b>EXECUTES</b><br>Moves, aims and fires from the state selected by Engagement.','soldier-v1');
}
