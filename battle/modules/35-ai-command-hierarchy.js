/* Command-hierarchy truth map for the Visual AI workbench.
   ...
   battle decisions. Force Command owns strategic squad intent. The physical squad leader is currently
...
var ui={root:null,view:null,world:null,nodes:null,svg:null,leader:null,observer:null,scheduled:false,drag:null,trace:null};
...
function savePosition(){if(!ui.leader)return;var p=nodeXY(ui.leader);try{localStorage.setItem(STORE,JSON.stringify(p));}catch(_){}}
...
function decorateCore(){
  ...
  if(squad){var hs=squad.querySelector('.ag-head>span'),bs=squad.querySelector('.ag-head small');if(hs)hs.textContent='Squad Orders';if(bs)bs.textContent='formation / orders';ownerBox(squad,'<span class="how">OWNS ORGANIZATION</span><br><b>formation · order anchor · member slots</b><br>Turns Force Command intent into stable squad/fireteam orders. Squad leader status modifies execution but does not create a competing mission.','squad-v1');}
...
function leaderBody(){var p=params(),coh=num(p.cohesionRadius,34),no=num(p.captainlessCohesion,26),extra=num(p.cornerNoCaptainExtra,.35);return'<div class="ag-muted"><b>Influence only — not a second order source.</b><br>The physical squad leader is currently a combatant and leadership/status input to Force Command and Squad Orders.</div><div class="ach-leader-status"><span>Leader alive</span><b>cohesion '+coh.toFixed(0)+' m</b><span>Leader KIA</span><b>cohesion '+no.toFixed(0)+' m</b><span>Corner penalty if KIA</span><b>+'+extra.toFixed(2)+' s</b><span>Policy condition</span><b>captainDead</b><span>Other current jobs</span><b>formation slot · voice</b></div>';}
function ensureLeader(){
  var existing=graphNode('leadership:squad-leader');if(existing){ui.leader=existing;var body=existing.querySelector('.ag-body');if(body)body.innerHTML=leaderBody();return existing;}
  var squad=graphNode('system:squad');if(!squad)return null;var sp=nodeXY(squad),saved=savedPosition()||{x:sp.x-85,y:sp.y+250};
  var el=document.createElement('div');el.className='ag-node leadership';el.dataset.k='leadership:squad-leader';el.style.left=saved.x+'px';el.style.top=saved.y+'px';el.innerHTML='<div class="ag-head"><span>Squad Leader</span><small>Staff Sergeant / Unteroffizier · influence / status</small></div><div class="ag-body">'+leaderBody()+'</div><i class="ag-sock out" title="Leadership influence"></i>';
  ui.nodes.appendChild(el);ui.leader=el;
  el.addEventListener('click',function(e){e.stopPropagation();traceLeader();});
...
function drawWires(){var svg=ensureSvg('agHierarchySvg',4);svg.innerHTML='';var cap=ensureLeader(),command=graphNode('system:commander'),squad=graphNode('system:squad');if(!cap)return;addWire(svg,cap,command,'leadership status / captainDead');addWire(svg,cap,squad,'cohesion / formation / voice');}
...
function drawTrace(){if(ui.trace!=='squad-leader')return;var svg=ensureSvg('agHierarchyTrace',55);svg.innerHTML='';var cap=ensureLeader(),command=graphNode('system:commander'),squad=graphNode('system:squad');if(!cap)return;[cap,command,squad].forEach(function(n){if(n)n.classList.add('ach-trace-node');});addWire(svg,cap,command,'STATUS INPUT');addWire(svg,cap,squad,'EXECUTION MODIFIER');}
function traceLeader(){
  if(root.BattleAILoopWatch&&root.BattleAILoopWatch.highlight)root.BattleAILoopWatch.highlight([]);clearTrace();ui.trace='squad-leader';drawTrace();
  var head=document.getElementById('agInspectorHead'),body=document.getElementById('agInspectorBody');if(head)head.innerHTML='<b>Squad Leader</b><small>Staff Sergeant / Unteroffizier · influence, not an order authority</small>';if(body)body.innerHTML='<div class="ag-section"><h3>Current responsibility</h3><div class="ag-tip">Squad leader survival changes cohesion, corner delay and the <b>captainDead</b> policy condition. The squad leader also occupies a formation slot and voices squad state changes. He does <b>not</b> independently choose Hold / Assault / Flank / Regroup.</div></div><div class="ag-section"><h3>Future responsibility · Step 5</h3><div class="ag-tip"><b>Force Command decides WHAT.</b><br>Squad Leader will decide HOW LOCALLY: approach side, formation, base of fire, bounding and occupation of prepared positions. He will not be allowed to replace the strategic objective.</div></div>';
}
function ensureKey(){if(document.getElementById('agHierarchyKey'))return;var k=document.createElement('div');k.id='agHierarchyKey';k.className='ach-key';k.innerHTML='<b>COMMAND OWNERSHIP</b><br><span class="what">Force Command = WHAT</span> · objective / route / phase<br><span class="how">Squad Leader = HOW locally (future)</span> · currently leadership influence only<br><span class="how">Engagement = HOW each soldier fights</span>';ui.view.appendChild(k);}
function decorate(){decorateCore();ensureLeader();ensureKey();drawWires();if(ui.trace)drawTrace();}
