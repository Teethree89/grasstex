/* Rolling squad forward-progress diagnostics.
   A squad can travel a long distance without actually getting closer to its assigned objective
   (circling, route churn, regroup yo-yos, local avoidance loops). Stalls alone miss that failure
   because everybody is technically moving. This system measures centroid travel against net
   progress toward the stable strategic target over a rolling window and feeds pathological
   low-progress movement into Loop Watch and the benchmark. */
(function(root){
'use strict';
...
function stableGoal(sim,sq){return objectiveById(sim,sq&&sq.targetObjective)||clonePoint(sq&&sq._routeFinalObjective)||clonePoint(sq&&sq.objective);}
...
function pushLoop(sim,sq,a){var lw=sim&&sim._aiLoopWatch;if(lw&&Array.isArray(lw.alerts))lw.alerts.push({kind:'low-forward-progress',severity:a.net<0?'hot':'warn',faction:sq.faction,squadId:sq.id,soldierId:null,time:a.at,travel:a.travel,net:a.net,destinationChanges:a.routeChanges||0,inContact:!!sq.inContact,phases:a.phases||[],rules:[],sequence:a.phases||[],message:'Squad travelled '+a.travel.toFixed(1)+'m but made only '+a.net.toFixed(1)+'m net objective progress in '+WINDOW+'s (eff '+Math.round(a.efficiency*100)+'%)'});}
function tickSquad(sim,sq){
  if(!sq||sq.state==='retreat')return;
  var ph=phase(sq);if(!ADVANCE[ph])return;
  var p=centroid(sq),g=stableGoal(sim,sq);if(!p||!g||!isFinite(dist(p,g)))return;
  var t=+sim.time||0,st=state(sim),k=key(sq),tr=st.tracks[k];
  if(!tr||dist(tr.goal,g)>8){tr=st.tracks[k]={goal:g,hist:[],last:p,lastRoute:+sq.routeIndex||0,routeChanges:0,lastAlert:-1e9};}
  ...
  st.samples++;if(travel>=MIN_TRAVEL&&eff<MIN_EFF)st.lowEfficiencySamples++;
  if(travel<MIN_TRAVEL||net>=MIN_NET||eff>=MIN_EFF||t-tr.lastAlert<ALERT_COOLDOWN)return;
  tr.lastAlert=t;
  var phases=[],seen={};for(var i=0;i<tr.hist.length;i++){var hph=tr.hist[i].phase;if(!seen[hph]){seen[hph]=1;phases.push(hph);}}
  var a={kind:'low-forward-progress',faction:sq.faction,squad:sq.id,at:+t.toFixed(2),window:WINDOW,travel:+travel.toFixed(2),net:+net.toFixed(2),efficiency:+eff.toFixed(3),distance:+last.d.toFixed(2),routeChanges:(last.routeChanges||0)-(first.routeChanges||0),phases:phases,spread:null};
  try{if(root.BattleCommanderDoctrine){var c=root.BattleCommanderDoctrine.avgPos(sq);a.spread=+root.BattleCommanderDoctrine.maxSpread(sq,c).toFixed(2);}}catch(_){}
  st.totalAlerts++;if(st.byFaction[sq.faction]!=null)st.byFaction[sq.faction]++;st.alerts.push(a);if(st.alerts.length>MAX_ALERTS)st.alerts.shift();
  pushLoop(sim,sq,a);
  if(root.BattleTelemetry&&root.BattleTelemetry.record)root.BattleTelemetry.record('low-forward-progress',a,sim);
}
...
root.BattleModules.registerSystem('squad-forward-progress',{version:'1.0',onBattleStart:reset,onBattleRestart:reset,onCommanderTick:tick});
root.BattleSquadForwardProgress={version:'1.0',windowSeconds:WINDOW,summary:function(sim){return sim&&sim._squadForwardProgressSummary?JSON.parse(JSON.stringify(sim._squadForwardProgressSummary)):null;}};
console.log('[AI] rolling squad forward-progress diagnostics active');
