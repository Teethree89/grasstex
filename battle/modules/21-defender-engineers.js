...
(function(root){
  ...
  function garrison(sim,sq,sector,plan,teleport){
    ...
    if(teleport){sq.commandRole='garrison';if(root.BattleSquadStability)root.BattleSquadStability.initialPhase(sq,'defend');else sq.commandPhase='defend';sq.targetObjective=sector.objectiveId;sq.objective=copy(point);sq.home=root.BattleSides.offsetPoint(point,frame,sector.radius*2.4,0);sq.route=[copy(center)];sq.routeIndex=0;sq.orderAnchor=copy(center);sq.rally=copy(center);sq._orderGoal=null;sq._formationForward={x:frame.front.x,z:frame.front.z};sq._stablePlan=null;sq._stablePlanSerial=(sq._stablePlanSerial||0)+1;sq._fireteamOrders={};}
