...
(function (root) {
  ...
  var oldStart = root.BattleSim.start,
    T = root.BattleSoldierModel.TAGS;
  function finiteAmmo() {
    return !!root.BattleAmmunition;
  }
  function startLegacyReload(s, battle) {
    if (finiteAmmo() || !s || s.dead || !s.weapon || s.reloading) return;
    var dur = s.weapon.stats.reloadTime || 2.5;
    s.reloading = true;
    ...
    if (root.BattleTelemetry)
      root.BattleTelemetry.record(
        'decision-reload',
        { soldier: s.id, faction: s.faction, weapon: s.weapon.kind, duration: dur },
        battle
      );
  }
  function finishLegacyReload(s) {
    if (finiteAmmo() || !s || !s.weapon) return;
    s.reloading = false;
    s.weapon.ammo = s.weapon.magSize || s.weapon.stats.magazine || 8;
    s.reloadStart = 0;
    s.reloadUntil = 0;
  }
    ...
    if (s && s.reloading) {
      ...
      if (s.target && root.BattleMovementResolver) {
        root.BattleMovementResolver.proposeCombat(
          s,
          { x: s.root.position.x, z: s.root.position.z },
          battle,
          'reload-hold',
          null,
          { source: 'weapon-cycle', reason: 'reload pause' }
        );
        root.BattleMovementResolver.resolve(s, battle);
  ...
  root.BattleSim.start = function (scene, opts) {
    ...
    sim.onFire = function (soldier) {
      if (soldier && soldier.weapon) {
        ...
        if (!finiteAmmo())
          soldier.weapon.ammo = Math.max(
            0,
            (soldier.weapon.ammo == null ? soldier.weapon.magSize : soldier.weapon.ammo) - 1
          );
  ...
  root.BattleModules.registerSystem('soldier-combat-animation', {
    version: '23-finite-ammo-owner',
