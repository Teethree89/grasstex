...
(function (root) {
  ...
  function units(sim) {
    return root.BattleModules.unitsFor(sim).filter(function (s) {
      return s && s.weapon && !s.dead;
    });
  }
  function freshBucket() {
    return { shots: 0, reloads: 0, stoppages: 0, clears: 0, ranDry: 0, lowAmmoCalls: 0, roundsLoaded: 0 };
  }
  function fresh() {
    return {
      shots: 0,
  ...
  function stats(sim) {
    return sim._ammunitionStats || (sim._ammunitionStats = fresh());
  }
  function bump(sim, s, field, n) {
    n = n == null ? 1 : n;
    var st = stats(sim),
      kind = (s && s.weapon && s.weapon.kind) || 'rifle',
      b = st.byWeapon[kind] || (st.byWeapon[kind] = freshBucket());
    st[field] = (st[field] || 0) + n;
    b[field] = (b[field] || 0) + n;
  }
  function deterministicCall(s, count, mod) {
    return ((+s.id || 0) + (+count || 0) * 3) % mod === 0;
  }
  function say(s, battle, type) {
    try {
      if (battle && battle.onCallout) battle.onCallout(s, type);
    } catch (_) {}
  }
  function animateReload(s, seconds) {
    try {
      var M = root.BattleSoldierModel;
      if (M && M.triggerAnimation && M.TAGS && M.TAGS.reload)
        M.triggerAnimation(s, M.TAGS.reload, { duration: seconds });
    } catch (_) {}
  }
  ...
  function carried(s) {
    return s && s.weapon ? Math.max(0, +s.weapon.ammo || 0) + Math.max(0, +s.weapon.reserveAmmo || 0) : 0;
  }
  function maybeLow(s, battle) {
    var a = s && s._ammoState,
      c = cfg(s);
    if (!a || a.lowCalled || carried(s) > c.low) return;
    a.lowCalled = true;
    bump(battle, s, 'lowAmmoCalls');
    say(s, battle, 'ammoLow');
  }
  function finishReload(s, battle) {
    var w = s.weapon,
      need = Math.max(0, (+w.magSize || 1) - (+w.ammo || 0)),
      load = Math.min(need, Math.max(0, +w.reserveAmmo || 0));
    w.ammo = (+w.ammo || 0) + load;
    w.reserveAmmo = Math.max(0, (+w.reserveAmmo || 0) - load);
    s.reloading = false;
    s.reloadUntil = 0;
    s.outOfAmmo = w.ammo <= 0 && w.reserveAmmo <= 0;
    if (load > 0) bump(battle, s, 'roundsLoaded', load);
    markFireReady(s, battle, 0.18);
    maybeLow(s, battle);
  }
  function startReload(s, battle) {
    if (!s || !s.weapon || s.reloading || s.clearingStoppage) return false;
    var w = s.weapon;
    if ((+w.ammo || 0) > 0 || (+w.reserveAmmo || 0) <= 0) return false;
    ...
    var a =
      ...
      (s._ammoState = { reloads: 0, stoppages: 0, shots: 0, lowCalled: false, dryCounted: false });
    a.reloads++;
    bump(battle, s, 'reloads');
    animateReload(s, seconds);
  ...
  function finishStoppage(s, battle) {
    ...
    s.clearingStoppage = false;
    s.stoppageUntil = 0;
    bump(battle, s, 'clears');
    markFireReady(s, battle, 0.2);
  }
  function unavailable(s, battle) {
    if (!s || !s.weapon) return true;
    var w = s.weapon;
    if (s.reloading || s.clearingStoppage || w.jammed) return true;
    if ((+w.ammo || 0) > 0) return false;
    if ((+w.reserveAmmo || 0) > 0) {
      startReload(s, battle);
      return true;
    }
    s.outOfAmmo = true;
    var a = s._ammoState;
  ...
  function afterShot(s, battle) {
    var w = s.weapon,
      ...
      a =
        s._ammoState ||
        (s._ammoState = { reloads: 0, stoppages: 0, shots: 0, lowCalled: false, dryCounted: false });
    w.ammo = Math.max(0, (+w.ammo || 0) - 1);
    w.heat = Math.min(1, (+w.heat || 0) + c.heatPerShot);
    a.shots++;
    bump(battle, s, 'shots');
    maybeLow(s, battle);
    if (w.ammo <= 0) {
      if ((+w.reserveAmmo || 0) > 0) startReload(s, battle);
  ...
  function ready(s, battle) {
    return !unavailable(s, battle);
  }
  ...
  function tick(sim, payload) {
    var dt = Math.max(0, payload && isFinite(+payload.dt) ? +payload.dt : 0),
      t = +sim.time || 0,
      a = units(sim);
    for (var i = 0; i < a.length; i++) {
      var s = a[i],
        w = s.weapon,
        c = cfg(s);
      if (!s._ammoState) initialize(s, sim);
      ...
      if (!s.reloading && !s.clearingStoppage && (+w.ammo || 0) <= 0 && (+w.reserveAmmo || 0) > 0)
        startReload(s, sim);
      maybeLow(s, sim);
    }
  }
  function reset(sim) {
    sim._ammunitionStats = fresh();
    var a = units(sim);
    for (var i = 0; i < a.length; i++) initialize(a[i], sim);
    publish(sim);
  }
  function snapshot(sim) {
    ...
    for (var i = 0; i < a.length; i++) {
      var w = a[i].weapon,
        l = Math.max(0, +w.ammo || 0),
        r = Math.max(0, +w.reserveAmmo || 0);
      loaded += l;
      reserve += r;
      remaining += l + r;
      if (l + r <= 0) empty++;
      if (a[i].reloading) reloading++;
      if (a[i].clearingStoppage) clearing++;
    }
    return {
      shots: st.shots,
      reloads: st.reloads,
      stoppages: st.stoppages,
      clears: st.clears,
      ranDry: st.ranDry,
      lowAmmoCalls: st.lowAmmoCalls,
      roundsLoaded: st.roundsLoaded,
      ...
      reloading: reloading,
      clearing: clearing,
      byWeapon: JSON.parse(JSON.stringify(st.byWeapon))
    };
  }
  function publish(sim) {
    var out = snapshot(sim);
    sim._ammunitionSummary = out;
    if (sim._coordinationHealth) sim._coordinationHealth.ammunition = JSON.parse(JSON.stringify(out));
  }

  root.BattleModules.registerSystem('ammunition-stoppages', {
    version: '1.1',
    onBattleStart: reset,
