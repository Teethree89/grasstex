...
(function(root){
'use strict';
if(!root.BattleModules||!root.BattleEngagement||!root.BattleEngagement.extend||!root.BattleMovementResolver||root.BattleCombatUrgency)return;
...
function bump(sim,field){var st=sim._combatUrgencySummary||(sim._combatUrgencySummary={coverRejects:0,lateralCoverRejects:0,urgentCoverStarts:0,urgentCoverArrivals:0,sharedContactReactions:0,sharedContactRepeatBlocks:0,urgentFrames:0});st[field]++;}
function request(s,p,b,kind,ttl,reason){return root.BattleMovementResolver.proposeCombat(s,p,b,kind,ttl,{source:'engagement',reason:reason});}
function allowCover(s,sim,pt){var q=s&&s.squad;if(!q||q.commandPhase!=='assault'||q.state==='retreat'||(+s.suppressedUntil||0)>sim.time)return true;var ax=axis(sim,q),here=pos(s);if(!ax||!here)return true;var fw=(pt.x-here.x)*ax.fx+(pt.z-here.z)*ax.fz;if(fw>=MIN_COVER_FORWARD)return true;bump(sim,'coverRejects');if(fw>=-.25)bump(sim,'lateralCoverRejects');return false;}
...
function startUrgent(s,b,e){if(!safeToMove(s,b)||(+s.suppressedUntil||0)<=b.time||!exposed(s,b))return false;var threat=threatFor(s,b);if(!threat||!root.BattleEngagement.findCover)return false;if(e._urgentCoverSearchAt&&b.time<e._urgentCoverSearchAt)return false;e._urgentCoverSearchAt=b.time+.9;var cover=root.BattleEngagement.findCover(s,b,{maxRange:COVER_SEARCH,threat:threat,minEnemyDistance:10});if(!cover)return false;e.cover=cover;e.state='bound';e.since=b.time;e.until=b.time+Math.max(2,cover.distance/Math.max(2.4,+s.crouchRunSpeed||+s.runSpeed||+s.speed||3)+1);e._urgentCover=true;s._combatUrgentUntil=b.time+URGENT_TTL;s.prone=false;s.crawling=false;s.tacticalCrouch=true;s.setUp=false;request(s,cover,b,'cover-bound',.8,'suppressed cover move');bump(b,'urgentCoverStarts');return true;}
function maintainUrgent(s,b,e){if(!e._urgentCover||!e.cover)return false;var d=dist(pos(s),e.cover);if(d<=COVER_ARRIVED){e._urgentCover=false;s._combatUrgentUntil=0;bump(b,'urgentCoverArrivals');return false;}if(!safeToMove(s,b)||e.state!=='bound'){e._urgentCover=false;s._combatUrgentUntil=0;return false;}s._combatUrgentUntil=b.time+URGENT_TTL;s.prone=false;s.crawling=false;s.tacticalCrouch=true;request(s,e.cover,b,'cover-bound',.8,'suppressed cover move');return true;}
function reactShared(s,b,e){var q=s&&s.squad;if(!q||!q.inContact){if(e._sharedContactQuietAt==null)e._sharedContactQuietAt=b.time;if(e._sharedContactAware&&b.time-e._sharedContactQuietAt>=SHARED_RESET_QUIET){e._sharedContactAware=false;e._sharedContactSector=null;e._sharedContactReactedAt=-999;}return;}e._sharedContactQuietAt=null;if(s.target||e.state!=='advance'||s.reloading||s.clearingStoppage)return;var c=contact(s,b);if(!c)return;var h=pos(s),aim={x:+c.x||0,z:+c.z||0};if(!h)return;var sec=sectorFor(s,aim);if(e._sharedContactAware&&sectorDistance(e._sharedContactSector,sec)<=1){bump(b,'sharedContactRepeatBlocks');return;}e._sharedContactAware=true;e._sharedContactSector=sec;e._sharedContactReactedAt=b.time;e.state='alert';e.since=b.time;e.until=b.time+SHARED_HOLD;e.lastSeen=aim;e.lastSeenAt=Math.max(+e.lastSeenAt||-999,+c.at||b.time);s._faceHint=aim;s.tacticalCrouch=true;request(s,h,b,'contact-reaction',Math.min(.8,SHARED_HOLD),'new shared threat');bump(b,'sharedContactReactions');}
/* Engagement's afterDrill slot runs this once the state machine has decided the soldier's tick. */
root.BattleEngagement.extend('afterDrill','combat-urgency',function(s,b){var e=estate(s);if(!maintainUrgent(s,b,e)&&(+s.suppressedUntil||0)>b.time&&['pinned','engage','orient'].indexOf(String(e.state||''))>=0)startUrgent(s,b,e);reactShared(s,b,e);});
function markUrgent(sim){var t=+sim.time||0,a=root.BattleModules.unitsFor(sim);for(var i=0;i<a.length;i++){var s=a[i];if(!s||s.dead||t>=(+s._combatUrgentUntil||0))continue;if(s.prone){s.prone=false;s.crawling=false;}s.tacticalCrouch=true;bump(sim,'urgentFrames');}}
function reset(sim){sim._combatUrgencySummary=null;var a=root.BattleModules.unitsFor(sim);for(var i=0;i<a.length;i++){a[i]._combatUrgentUntil=0;if(a[i].eng){a[i].eng._urgentCover=false;a[i].eng._urgentCoverSearchAt=0;a[i].eng._sharedContactAware=false;a[i].eng._sharedContactSector=null;a[i].eng._sharedContactQuietAt=null;}}}
root.BattleModules.registerSystem('combat-urgency',{version:'1.3-resolver-coalesced',onBattleStart:reset,onBattleRestart:reset,onSimulationStep:markUrgent});
root.BattleAssaultForwardGuard={version:'1.3-resolver-coalesced',allowCover:allowCover};
root.BattleCombatUrgency={version:'1.3-resolver-coalesced',summary:function(sim){return sim&&sim._combatUrgencySummary?JSON.parse(JSON.stringify(sim._combatUrgencySummary)):null;}};
