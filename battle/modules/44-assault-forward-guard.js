/* Lean combat-mobility owner.
   ...
   it may request a movement intent, but only this owner publishes combat movement to the resolver.
   Repeated identical intents are coalesced here instead of making the resolver reject the same
   request every AI tick. */
(function(root){
'use strict';
...
/* Compatibility boundary for the existing engagement state machine. It no longer reaches the
   resolver as an independent writer: every legacy engagement proposal is absorbed by the sole
   combat-mobility owner and coalesced before publication. */
root.BattleMovementResolver.proposeCombat=function(s,p,b,kind,ttl,meta){
  if(meta&&meta.source==='engagement')return request(s,p,b,kind,ttl,{origin:'engagement',reason:meta.reason||kind});
  return baseProposeCombat.apply(this,arguments);
};
...
root.BattleEngagement.updateSoldier=function(s,b){
  if(s&&b&&!s.dead){var before=estate(s);/* Rare benchmark edge: a target can die between deciding to assault and constructing the first rush point. Do not enter the private assault routine without either a live target or an already-committed goal. */if(before.state==='assault'&&!before.assaultGoal&&(!s.target||s.target.dead)){before.state='alert';before.since=b.time;before.until=b.time+SHARED_HOLD;before.moveReason='assault target unavailable';}}
  var result=oldUpdate.apply(this,arguments);if(!s||!b||s.dead)return result;maintainPush(s,b);var e=estate(s);if(!maintainUrgent(s,b,e)&&(+s.suppressedUntil||0)>b.time&&['pinned','engage','orient'].indexOf(String(e.state||''))>=0)startUrgent(s,b,e);reactShared(s,b,e);return result;
};
