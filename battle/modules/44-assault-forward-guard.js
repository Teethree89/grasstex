...
function reset(sim){sim._combatMobilityStats=fresh();var a=root.BattleModules.unitsFor(sim);for(var i=0;i<a.length;i++){delete a[i]._combatMobilityIntent;a[i]._combatUrgentUntil=0;if(a[i].eng){a[i].eng._urgentCover=false;a[i].eng._urgentCoverSearchAt=0;a[i].eng._sharedContactAware=false;a[i].eng._sharedContactSector=null;a[i].eng._sharedContactQuietAt=null;}}}
function publish(sim){var st=JSON.parse(JSON.stringify(stats(sim)));sim._combatMobilitySummary=st;sim._assaultForwardGuardSummary={orderClamps:st.orderClamps,coverRejects:st.coverRejects,lateralCoverRejects:st.lateralCoverRejects};sim._assaultBoundMomentumSummary={pushes:st.pushes,completions:st.pushCompletions,cancels:st.pushCancels,pushMeters:6.5};sim._combatUrgencySummary={urgentCoverStarts:st.urgentCoverStarts,urgentCoverArrivals:st.urgentCoverArrivals,sharedContactReactions:st.sharedContactReactions,sharedContactRepeatBlocks:st.sharedContactRepeatBlocks,urgentFrames:st.urgentFrames};}
