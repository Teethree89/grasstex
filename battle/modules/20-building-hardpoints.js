...
function fresh(){return{assignmentsCreated:0,assignmentsOccupied:0,assignmentsReleased:0,releaseReasons:{},reassignments:0,claimCollisionsPrevented:0,ingressRoutesCreated:0,ingressRoutesInvalidated:0,releasedLifetime:0,assignmentsByRole:{},captainWindowAssignments:0};}
function context(sim){var c=contexts.get(sim);if(!c){c={live:new Map(),reservations:new Map(),history:[],serial:0,revision:0,stats:fresh()};contexts.set(sim,c);}return c;}
function current(s){var a=s&&assignments.get(s);return a?a.task:null;}
function station(s){var t=current(s);return t?t.position:null;}
...
function release(s,sim,reason){
  var a=s&&assignments.get(s);if(!a)return false;
  sim=a.sim;var c=context(sim),t=a.task;reason=reason||'explicit-task-change';
  t.status='released';t.releasedAt=sim.time;t.releaseReason=reason;t.lifetime=Math.max(0,sim.time-t.assignedAt);
  c.revision++;c.reservations.delete(t.station);c.live.delete(s);assignments.delete(s);
  c.stats.assignmentsReleased++;c.stats.releaseReasons[reason]=(c.stats.releaseReasons[reason]||0)+1;c.stats.releasedLifetime+=t.lifetime;
  c.history.push(snapshot(t));if(c.history.length>80)c.history.shift();
  s._nextStationClaimAt=sim.time+CLAIM_RETRY;s._navCache=null;s._physicalPath=null;s._faceHint=null;s.setUp=false;
  if(s._movementResolver)s._movementResolver.combat=null;
  if(s.eng&&s.eng.state==='station'){s.eng.state='advance';s.eng.cover=null;s.eng.setUpSince=0;}
  emit(sim,'released',t,{reason:reason,lifetime:t.lifetime});return true;
}
...
function claim(s,sim,st,threat){
  ...
  var existing=current(s);if(existing)return existing;
  ...
  s._navCache=null;s._physicalPath=null;s._nextStationClaimAt=0;
  if(s._movementResolver)s._movementResolver.combat=null;
  if(s.eng){s.eng.cover=null;s.eng.boundOrder=false;s.eng.suppressOrder=false;}
  ...
  if(!t.route){c.stats.ingressRoutesInvalidated++;release(s,sim,'station-unreachable');return null;}
  c.stats.ingressRoutesCreated++;t.status='ingress';emit(sim,'ingress',t,{door:t.route.door});return t;
}
function select(s,sim,threat){
  var c=context(sim),here=s.root.position,range=s.role==='gunner'?78:62;
  var candidates=N.firingStations.filter(function(st){
    if(distance(here,st)>range)return false;
    var dx=threat.x-st.windowX,dz=threat.z-st.windowZ,len=Math.hypot(dx,dz)||1;
    if((dx*st.normalX+dz*st.normalZ)/len<.32)return false;
    if(c.reservations.has(st.id)){c.stats.claimCollisionsPrevented++;return false;}
  ...
  candidates.sort(function(a,b){return distance(here,a)-distance(here,b);});
  for(var i=0;i<Math.min(2,candidates.length);i++){var t=claim(s,sim,candidates[i],threat);if(t)return t;}
  return null;
}
function maintain(sim){context(sim).live.forEach(function(t,s){update(s,sim);});}
function assign(sim){
  maintain(sim);
  ['us','ge'].forEach(function(f){(sim.factions[f].squads||[]).forEach(function(sq){
    var count=sq.members.filter(function(s){return!!current(s);}).length;if(count>=MAX_PER_SQUAD)return;
    var members=sq.members.slice().sort(function(a,b){return(a.role==='gunner'?0:1)-(b.role==='gunner'?0:1);});
    for(var i=0;i<members.length;i++){
      var s=members[i];if(!canAssign(s)||current(s)||s._nextStationClaimAt>sim.time)continue;
      var R=root.BattleTacticalRoute,threat=R&&R.knownThreat(s,sim);if(!threat)continue;
      var t=select(s,sim,threat);if(!t)s._nextStationClaimAt=sim.time+CLAIM_RETRY;
...
function waypoint(s,sim){
  var t=update(s,sim);if(!t)return null;var r=t.route,here=s.root.position;
  while(r.index<r.steps.length-1&&distance(here,r.steps[r.index])<=.85&&N.movementClear(here,r.steps[r.index+1]))r.index++;
  return{point:point(r.steps[r.index]),reason:'position-ingress',intent:point(t.position),step:r.index,total:r.steps.length};
}
function summary(sim){
  var c=context(sim),out=Object.assign({},c.stats),live=[],lifetime=c.stats.releasedLifetime;
  c.live.forEach(function(t){live.push(snapshot(t));lifetime+=Math.max(0,sim.time-t.assignedAt);});
  out.averageAssignmentLifetime=c.stats.assignmentsCreated?lifetime/c.stats.assignmentsCreated:0;
  out.averageReleasedLifetime=c.stats.assignmentsReleased?c.stats.releasedLifetime/c.stats.assignmentsReleased:0;
  out.currentLiveAssignments=live.length;out.live=live;out.recentReleases=c.history;return snapshot(out);
}
