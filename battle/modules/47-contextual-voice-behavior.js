...
function phaseVoice(sim,sq,ss){var phase=String(sq.commandPhase||''),old=ss.phase,t=+sim.time||0;if(old===null){ss.phase=phase;ss.orderVersion=+sq._orderVersion||0;return;}if(phase!==old){var cap=captain(sq),commandSpoken=false;if(phase==='regroup')commandSpoken=say(sim,cap,'regroup',{cooldown:10});else if((phase==='assault'||phase==='flank')&&['hold','support-hold','defend','regroup','reserve'].indexOf(old)>=0&&chance(sim,key(sq)+'|push|'+Math.floor(t),.55))commandSpoken=say(sim,cap,'pushNow',{cooldown:12});var ack=+sequence('orderAck').chance||.45;if(chance(sim,key(sq)+'|ack|'+old+'>'+phase+'|'+Math.floor(t),ack)){var s=pick(sq,'ack|'+phase+'|'+Math.floor(t),true,cap?[String(cap.id)]:null);schedule(sim,sq,s,'orderAck',commandSpoken?2850:900,{cooldown:6});}ss.phase=phase;}
var rh=sq._regroupHysteresis,uses=rh&&(+rh.rallyUses||0)||0;if(uses>ss.rallyUses){var c=captain(sq);if(c)say(sim,c,'rallyHere',{cooldown:12});ss.rallyUses=uses;}ss.orderVersion=+sq._orderVersion||0;}
