/* Contextual infantry voice behavior.
...
var SYSTEM='contextual-infantry-voice';
var SAMPLE=.45,CONTACT_GRACE=12,TACTICAL_GAP=8,STORY_GLOBAL_GAP=9;

function wallNow(){return root.performance&&typeof root.performance.now==='function'?root.performance.now():Date.now();}
...
function state(sim){return sim._contextVoice||(sim._contextVoice={squads:Object.create(null),owners:Object.create(null),killSerial:0,lastSocialSim:-1e9,nextSample:0,pending:[]});}
function squadState(sim,sq){var all=state(sim).squads,k=key(sq);return all[k]||(all[k]={phase:null,orderVersion:+sq._orderVersion||0,rallyUses:0,contact:null,lastMovementCall:-1e9,lastWindowCall:-1e9,lastFlankCall:-1e9,lastTakingFire:-1e9,lastPinned:-1e9,lastBoundCall:-1e9,lastStory:-1e9,lastQuip:-1e9,socialSince:null,suppressedRatio:0,eng:Object.create(null),events:Object.create(null)});}
function manifest(){return root.BATTLE_AUDIO_MANIFEST||null;}
...
function pick(sq,seed,preferNonLeader,exclude){var a=alive(sq),pool=a.filter(function(s){return (!preferNonLeader||!(root.SquadAI&&root.SquadAI.isLeader(s)))&&(!exclude||exclude.indexOf(String(s.id))<0);});if(!pool.length)pool=a.filter(function(s){return !exclude||exclude.indexOf(String(s.id))<0;});if(!pool.length)return null;return pool[hash(key(sq)+'|'+seed)%pool.length];}
...
function observerForContact(sq){var c=sq&&sq.contact;return c&&memberById(sq,c.seenBy)||leader(sq)||pick(sq,'observer',true);}
function observeContact(sim,sq,ss){var c=sq.contact,t=+sim.time||0;if(!c||!c.unit||c.unit.dead||t-(+c.at||0)>CONTACT_GRACE){ss.contact=null;return;}var cur={id:String(c.unit.id),x:+c.x,z:+c.z,at:t};var speaker=observerForContact(sq);if((root.BattleTacticalPositions&&root.BattleTacticalPositions.station(c.unit))&&t-ss.lastWindowCall>15){var slot=(root.BattleTacticalPositions&&root.BattleTacticalPositions.station(c.unit)),event=(+slot.yBottom||0)>=1.8?'enemyInHouseUpper':(chance(sim,key(sq)+'|house|'+cur.id+'|'+Math.floor(t/10),.42)?'enemyInHouseGround':'enemyAtWindow');if(say(sim,speaker,event,{cooldown:15})){ss.lastWindowCall=t;}}
var f=forward(sq),center=average(sq);if(center){var vx=cur.x-center.x,vz=cur.z-center.z,vl=Math.hypot(vx,vz)||1,front=(vx*f.x+vz*f.z)/vl,lateral=(vx*f.z-vz*f.x)/vl;if((front<.35||Math.abs(lateral)>.78)&&t-ss.lastFlankCall>18){if(say(sim,speaker,'enemyFlanking',{cooldown:18}))ss.lastFlankCall=t;}}
if(ss.contact&&ss.contact.id===cur.id&&t-ss.contact.at>=1.1){var dx=cur.x-ss.contact.x,dz=cur.z-ss.contact.z,m=Math.hypot(dx,dz),rightX=f.z,rightZ=-f.x,lat=dx*rightX+dz*rightZ,along=dx*f.x+dz*f.z;if(m>=4.2&&Math.abs(lat)>=3.2&&Math.abs(lat)>Math.abs(along)*.75&&t-ss.lastMovementCall>12){var ev=lat>0?'enemyMovingRight':'enemyMovingLeft';if(say(sim,speaker,ev,{cooldown:12}))ss.lastMovementCall=t;}ss.contact=cur;}else if(!ss.contact||ss.contact.id!==cur.id)ss.contact=cur;}
function observeSuppression(sim,sq,ss){var a=alive(sq),t=+sim.time||0;if(!a.length)return;var sup=[],i;for(i=0;i<a.length;i++)if((+a[i].suppressedUntil||0)>t)sup.push(a[i]);var ratio=sup.length/a.length;if(sup.length&&ss.suppressedRatio===0&&t-ss.lastTakingFire>10){var who=sup[hash(key(sq)+'|fire|'+Math.floor(t))%sup.length];if(say(sim,who,'takingFire',{cooldown:10}))ss.lastTakingFire=t;}if(ratio>=.45&&ss.suppressedRatio<.45&&t-ss.lastPinned>14){var lead=leader(sq)||sup[0];if(say(sim,lead,'pinned',{cooldown:14}))ss.lastPinned=t;}ss.suppressedRatio=ratio;}
function observeEngagement(sim,sq,ss){var a=alive(sq),t=+sim.time||0;for(var i=0;i<a.length;i++){var s=a[i],e=engagementOf(s),cur=String(e.state||''),prev=ss.eng[String(s.id)]||'';if(cur!==prev){if(cur==='bound'&&sq.inContact&&t-ss.lastBoundCall>8&&chance(sim,key(sq)+'|bound|'+s.id+'|'+Math.floor(t),.38)){if(say(sim,s,'movingCover',{cooldown:8}))ss.lastBoundCall=t;}else if(prev==='bound'&&['station','engage','suppress','alert'].indexOf(cur)>=0&&sq.inContact&&t-ss.lastBoundCall>5&&chance(sim,key(sq)+'|set|'+s.id+'|'+Math.floor(t),.34)){if(say(sim,s,'coverSet',{cooldown:8}))ss.lastBoundCall=t;}ss.eng[String(s.id)]=cur;}}}
function phaseVoice(sim,sq,ss){var phase=String(sq.commandPhase||''),old=ss.phase,t=+sim.time||0;if(old===null){ss.phase=phase;ss.orderVersion=+sq._orderVersion||0;return;}if(phase!==old){var cap=leader(sq),commandSpoken=false;if(phase==='regroup')commandSpoken=say(sim,cap,'regroup',{cooldown:10});else if((phase==='assault'||phase==='flank')&&['hold','support-hold','defend','regroup','reserve'].indexOf(old)>=0&&chance(sim,key(sq)+'|push|'+Math.floor(t),.55))commandSpoken=say(sim,cap,'pushNow',{cooldown:12});var ack=+sequence('orderAck').chance||.45;if(chance(sim,key(sq)+'|ack|'+old+'>'+phase+'|'+Math.floor(t),ack)){var s=pick(sq,'ack|'+phase+'|'+Math.floor(t),true,cap?[String(cap.id)]:null);schedule(sim,sq,s,'orderAck',commandSpoken?2850:900,{cooldown:6});}ss.phase=phase;}
var rh=sq._regroupHysteresis,uses=rh&&(+rh.rallyUses||0)||0;if(uses>ss.rallyUses){var c=leader(sq);if(c)say(sim,c,'rallyHere',{cooldown:12});ss.rallyUses=uses;}ss.orderVersion=+sq._orderVersion||0;}
