...
void main(){if(vD>${SHADOW_END.toFixed(1)})discard;if(vD<${NEAR_SHADOW_END.toFixed(1)}&&uDrawNear<0.5)discard;if(vD>=${NEAR_SHADOW_END.toFixed(1)}&&uDrawMedium<0.5)discard;vec4 g=texture2D(grassTexture,vUV);if(g.a<.10)discard;float distanceFade=1.0-smoothstep(55.0,${SHADOW_END.toFixed(1)},vD);float a=g.a*(.17+.14*distanceFade);if(a<.016)discard;gl_FragColor=vec4(.032,.034,.041,a);}`;
  ...
  function startShadowJob(den){
    ...
    for(var v=0;v<6;v++){fit(mats,v,list.length*c6[v]*16);fit(seeds,v,list.length*c6[v]);matN[v]=seedN[v]=0;}job={list:list,i:0,count:count,commit:0};builtX=ox;builtZ=oz;builtDen=den;
  }
  ...
  function bakeShadowChunk(e,count){
    visit(e.x,e.z,count,function(i,gx,gz,yaw,gs,gh,seed,terrain){
      var v=i%6,a=mats[v],o=matN[v],gy=terrain&&Number.isFinite(+terrain.height)?+terrain.height:0;
