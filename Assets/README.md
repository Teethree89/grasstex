...
- soldiers/us-paratrooper.fbx, soldiers/ge-paratrooper.fbx (default rigged characters)
- soldiers/us-rifleman-rigged.fbx, soldiers/ge-rifleman-rigged.fbx (earlier characters, `?soldiers=rifleman`)
- weapons/m1-garand.fbx, weapons/kar98k.fbx (prepared rifles; the .zip files are the untouched Meshy sources)
- animations/*.fbx (animation-only Mixamo rifle clips on the same rig; loaded by name)

The deploy plan (`scripts/prepare_incremental_deploy.py`) uploads the `.fbx` files in these folders by content hash.
