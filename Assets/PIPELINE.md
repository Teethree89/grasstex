...
3. Measure where the hands go (weapon-local metres: right-hand `grip` on the wrist of the stock or
   ...
   The runtime seats `grip` on the model's right hand web and a reachable point on `fore` at its
   left index finger's second knuckle, blended slightly toward the palm web. If a model's posed
   hand spacing exceeds that weapon's generic fore-end range,
