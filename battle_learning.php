...
function arrv($a,$k,$default=null){return is_array($a)&&isset($a[$k])?$a[$k]:$default;}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $clean=array('id'=>'exp-'.gmdate('YmdHis').'-'.substr(hash('sha256',json_encode(array($seed,microtime(true)))),0,10),'storedAt'=>gmdate('c'),'seed'=>substr($seed,0,100),'scenarioId'=>substr(strval(arrv($e,'scenarioId','')),0,100),'trainingSeed'=>substr(strval(arrv($e,'trainingSeed','')),0,100),'fingerprint'=>$fp,'genome'=>$genome,'score'=>is_numeric(arrv($e,'score',null))?floatval(arrv($e,'score',0)):0,'winner'=>substr(strval(arrv($e,'winner','none')),0,20),'revision'=>intval(arrv($e,'revision',0)),'candidateId'=>substr(strval(arrv($e,'candidateId','')),0,80));
...
if($view==='memory'){$memory['ok']=true;$memory['experiences']=array_slice($memory['experiences'],-180);echo json_encode($memory,JSON_UNESCAPED_SLASHES);exit;}
if($view!=='dashboard'){http_response_code(400);echo json_encode(array('ok'=>false,'error'=>'unknown view'));exit;}

$days=max(1,min(30,isset($_GET['days'])?intval($_GET['days']):14));$files=is_dir($logDir)?glob($logDir.'/battle-decisions-*.jsonl'):array();if($files===false)$files=array();rsort($files,SORT_STRING);$files=array_slice($files,0,$days);
$matches=array();$training=array();$promotions=array();$recalls=array();$scenarioStats=array();$eventCounts=array();
foreach($files as $path){$fh=@fopen($path,'rb');if(!$fh)continue;while(($line=fgets($fh))!==false){$e=json_decode(trim($line),true);if(!is_array($e))continue;$type=strval(arrv($e,'type','unknown'));$eventCounts[$type]=isset($eventCounts[$type])?$eventCounts[$type]+1:1;$d=isset($e['data'])&&is_array($e['data'])?$e['data']:array();$base=array('time'=>isset($e['serverTime'])?$e['serverTime']:(isset($e['clientTime'])?$e['clientTime']:null),'session'=>arrv($e,'session',null),'battleTime'=>arrv($e,'battleTime',null));
    if($type==='policy-match-result'){if(count($matches)>=160)array_shift($matches);$matches[]=array_merge($base,$d);$seed=strval(arrv($d,'scenarioSeed',''));if($seed!==''){if(!isset($scenarioStats[$seed]))$scenarioStats[$seed]=array('seed'=>$seed,'matches'=>0,'scoreSum'=>0,'wins'=>0,'losses'=>0);$scenarioStats[$seed]['matches']++;$scenarioStats[$seed]['scoreSum']+=floatval(arrv($d,'score',0));$cf=arrv($d,'candidateFaction','');$w=arrv($d,'winner','none');if($w===$cf)$scenarioStats[$seed]['wins']++;elseif($w!=='none'&&$w!=='draw')$scenarioStats[$seed]['losses']++;}}
...
foreach($scenarioStats as $key=>$s)$scenarioStats[$key]['avgScore']=$s['matches']?round($s['scoreSum']/$s['matches'],3):0;
usort($matches,function($a,$b){return strcmp(strval(arrv($a,'time','')),strval(arrv($b,'time','')));});usort($training,function($a,$b){return strcmp(strval(arrv($a,'time','')),strval(arrv($b,'time','')));});
$policy=read_json_file($policyFile,array('revision'=>0,'genome'=>null,'score'=>null));$history=array();foreach(jsonl_records($historyFile,80) as $h){$meta=isset($h['meta'])&&is_array($h['meta'])?$h['meta']:array();$history[]=array('revision'=>arrv($h,'revision',0),'score'=>arrv($h,'score',null),'trainedAt'=>arrv($h,'trainedAt',null),'matches'=>arrv($h,'matches',null),'candidateId'=>arrv($meta,'candidateId',null),'trainingSeed'=>arrv($meta,'trainingSeed',null),'genome'=>arrv($h,'genome',null));}
$experiences=array_slice($memory['experiences'],-80);usort($experiences,function($a,$b){$as=floatval(arrv($a,'score',0));$bs=floatval(arrv($b,'score',0));if($as==$bs)return 0;return($as<$bs)?1:-1;});
$out=array('ok'=>true,'days'=>$days,'generatedAt'=>gmdate('c'),'policy'=>$policy,'policyHistory'=>$history,'matches'=>$matches,'trainingSummaries'=>$training,'promotions'=>$promotions,'recalls'=>$recalls,'scenarioStats'=>array_values($scenarioStats),'experiences'=>$experiences,'memoryCount'=>count($memory['experiences']),'eventCounts'=>$eventCounts);echo json_encode($out,JSON_UNESCAPED_SLASHES);
?>
