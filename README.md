...
`battle/battle_sim.html` is the modular page template. The supported entry point is
**`battle_sim.php`**, which resolves a GitHub commit, serves that template, and injects the
complete pinned runtime stack. It falls back to `battle_sim_local.php` when a full local deploy
is present and GitHub is temporarily unavailable. The root `battle_sim.html` only forwards to
that entry point, preventing an older, partial loader from silently running a different build.
